 ...
 *
 * This check only compares the pins against each other. `lint-runtime` checks the root pin's
 * hash against the registry and prints the corrected line to paste in, so a bump is: edit the
 * root `packageManager`, run `lint-runtime` for the hash, then copy it to the other manifests.
 */
