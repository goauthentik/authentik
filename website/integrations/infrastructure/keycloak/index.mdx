...
> Keycloak is an open-source IAM platform. It can both be used as an Identity Provider and as an application. This guide is for configuring Keycloak as an application.
>
> -- https://keycloak.org
...
- `keycloak.company` is the FQDN of your Keycloak company portal.
...
2. Navigate to **Configure** > **Identity Providers** and click **OpenID Connect v1.0**.
...
4. Click **Submit**.
5. Verify that the following options are properly configured in Keycloak after submitting:
    - **Service provider entity ID** is set to `https://keycloak.company/realms/<realm>`.
    - **Identity provider entity ID** is set to `authentik`.
    - **Single Sign-On Service URL** is set to `https://authentik.connorpeshek.me/application/saml/keycloak-saml/sso/binding/post/`.
    - **Single logout service URL** is set to `https://authentik.connorpeshek.me/application/saml/keycloak-saml/slo/binding/post/`.
