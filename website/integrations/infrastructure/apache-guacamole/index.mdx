...
:::note Guacamole Single Log Out (SLO) support
Only Guacamole 1.6.? and higher support SLO. If you're running an older version of Guacamole, do not include post logout related settings in authentik or Guacamole.
:::
...
2. Navigate to **Applications** > **Applications** and click **New Application** to create an application and provider pair. (Alternatively you can first create a provider separately, then create the application and connect it with the provider.)
    ...
    - **Configure the Provider**: provide a name (or accept the auto-provided name), the authorization flow to use for this provider, and the following required configurations.
        ...
        - Add a **Redirect URI** of type `Strict` `Post Logout` as `https://guacamole.company/`. If users access Apache Guacamole on a subpath, use the full URL that users enter in their browser.
...
Add the following variables to your Apache Guacamole environment file:
    ...
    OPENID-LOGOUT-ENDPOINT=https://authentik.company/application/o/<application_slug>/end-session/
    OPENID-POST-LOGOUT-REDIRECT-URI=https://guacamole.company/
    ```
