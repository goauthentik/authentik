...
Several modules exist, and they differ in how much they verify:
...
These notes describe each project as of August 2026, and all three are developed independently of PostgreSQL, so check the current state of whichever you choose. This guide uses Percona's module. Confirm that your choice verifies token signatures, and review it before using it in production.
...
`libpq`'s built-in flow uses the OAuth device authorization grant, which requires a device code flow on the brand that serves `authentik.company`. The flow is set per brand, so if one is already configured, skip this section; otherwise follow [Create and apply a device code flow](/docs/add-secure-apps/providers/oauth2/device_code/#create-and-apply-a-device-code-flow).
...
PostgreSQL checks every `oauth` entry in `pg_hba.conf` against `oauth_validator_libraries` while reading the file, and reports `oauth_validator_libraries must be set for authentication method oauth` when no validator is configured. At startup this is fatal and the server does not come up. On a running server it is silent: PostgreSQL logs `pg_hba.conf was not reloaded` and keeps the rules it already had, so the edit appears to have been ignored.
...
This maps `alice@company.com` to the `alice` role. Adjust the pattern to your domain and naming, and create the roles in PostgreSQL beforehand. Reload PostgreSQL once both files are in place.
