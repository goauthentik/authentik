...
5. Click **Finish** to save. Then, repeat the process to create a mapping for the user's *group memberships*:
...
6. Click **Finish** to save. Finally, repeat the process to create a mapping for the user's *username*:
...
4. Select `Metadata URL` as the **Configuration Method**, then paste the metadata URL that you copied from authentik.
5. Click **Load Info**, then click **Send**.
6. In the **Actions** column, click on the **Details** button next to the newly created authentik SAML connection.
7. Take note of the **Entity ID**, **Omada ID**, **Resource ID**, then click **OK**. These values will be required in the next section.
8. At the top right of the page, click **Go To SAML Role**, then **Add New SAML Role**.
...
You can generate this value using one of the following methods:
