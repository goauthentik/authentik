...
3. Navigate to **Settings** > **LDAP user and group backend** and configure the following settings:
    ...
    - On the **Server** tab:
        ...
        - Click the **+** icon and enter the following settings:
            - **Host**: enter the hostname/IP address of the authentik LDAP outpost preceded by `ldap://` or `ldaps://`. If using LDAPS you will also need to defint the certificate that is being used.
