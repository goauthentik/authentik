...
:::info
If something goes wrong with the configuration, you can use the URL `http://nextcloud.company/login?direct=1` to log in using the built-in authentication.
:::
...
3. Click the **+** button and enter the following settings:
    ...
    - **Scope**: `email profile openid` (omit `openid` if preferred)
    ...
    - **Use unique user ID**: If deselected, Nextcloud uses the mapped user ID in the Federated Cloud ID.
    ...
    :::danger
    If you are using a the `Nextcloud Profile` property mapping and want administrators to be able to log in, ensure that **Use unique user ID** is deselected. Otherwise, this setting will remove administrator users from the internal admin group and replace them with a hashed group ID named "admin", which lacks actual administrator access rights.
    :::
...
To confirm that authentik is properly configured with Nextcloud, log out and log back in using the **OpenID Connect** option. You will be redirected to authentik to log in, if succesful you will then be redirected to the Nextcloud dashboard.
...
To configure group quotas you'll need to create groups in authentik for each quota, and a property mapping.
...
3. Navigate to **Settings** > **SSO & SAML Authentication** and configure the following settings:
...
To confirm that authentik is properly configured with Nextcloud, log out and log back in using the **SSO and SAML log in** option. You will be redirected to authentik to log in, if succesful you will then be redirected to the Nextcloud dashboard.
...
3. Navigate to **Settings** > **LDAP user and group backend** and configure the following settings:
    ...
    - On the **Server** tab:
        ...
        - Click the **+** icon and enter the following settings:
            - **Host**: enter the hostname/IP address of the authentik LDAP outpost preceded by `ldap://`
            - **Port**: `389`
            - For **Credentials** enter the **Bind DN** of the authentik LDAP provider and the associated user password.
            - For **Base DN** enter the **Search base** of the authentik LDAP provider.

    - On the **Users** tab: - Set **Only these object classes** to `Users`. - On the **LDAP/AD integration** tab:
...
To confirm that authentik is properly configured with Nextcloud, log out and log back in using LDAP credentials. If succesful you will then be redirected to the Nextcloud dashboard.
