...
4. Once configured, single sign-on (SSO) login via authentik becomes available. TEMP
...
1. Create a group for each storage quota level and assign a custom attribute (e.g., `nextcloud_quota`) with values like `15 GB`.
...
## Create admin group _(optional)_

To grant Nextcloud admin access to authentik users:
...
3. Navigate to **Settings** > **SSO & SAML Authentication** and configure the following settings:
    ...
    - _(optional)_ **Attribute to map the quota to**: `nextcloud_quota` (leave blank if the `Nextcloud Profile` property mapping was skipped)
