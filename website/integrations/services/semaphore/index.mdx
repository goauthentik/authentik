...
Start the wizzard for adding a new application.
...
:::info
The name of the oidc_provider (e.g. `authentik`) need to match the name on the redirect url.
:::

:::info
If there is a `Not Found` error shown after the login and redirect to `https://semaphore.company/api/auth/oidc/authentik/`, it can help to set the web_root to `/`: 
