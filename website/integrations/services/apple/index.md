...
> Apple School Manager is designed for educational institutions to manage devices, apps, and accounts.
> It integrates with Student Information Systems (SIS) to automatically create and manage Apple IDs for students and staff, while providing specialized features for classroom environments and educational workflows.
>
> -- [Apple School Manager](https://www.apple.com/education/k12/deploy-and-manage/)

### Apple Business Essentials

> Apple Business Essentials is a subscription-based service that combines device management, storage, and support into a single package for small businesses.
> Unlike Apple Business Manager, it includes built-in mobile device management capabilities without requiring third-party MDM solutions, making it ideal for organizations with limited IT resources.
>
...
Before you can proceed with the Apple Business Manager configuration, you'll need to configure authentik to act as an identity provider for Apple. Start by logging into your authentik instance and navigating to the Admin interface.
...
Next, create a new scope mapping to handle read access to the user's profile information:
...
You'll need these values when configuring Apple Business Manager.
...
Visually confirm that you've copied the **Client ID** and **Client Secret** before pressing **Finish**. You'll need these values when configuring Apple Business Manager. Save the provider and confirm that the new provider is listed in the provider overview.

:::tip{title="OpenID Configuration URL"}

Before leaving the provider overview page, make sure to copy the **OpenID Configuration URL** value to your text editor. You'll need this when configuring Apple Business Manager.

You can always find your provider's generated URLs by navigating to **Providers**, then selecting the provider by name. The URLs are listed under the **Overview** tab.

:::

### Shared Signals Framework Provider

While the OAuth2/OpenID Provider handles the authentication flow, you'll need to create a Shared Signals Framework Provider to handle the backchannel communication between authentik and Apple Business Manager.
...
You're now ready to create the application configuration for Apple Business Manager. From the authentik Admin interface, navigate to **Applications** -> **Applications** and press **Create**.
...
With the application created, you're almost ready to switch over to Apple Business Manager.

:::tip{title="SSF Config URL"}

Assigning an SSF provider to a application will generate _SSF Config URL_ value. You'll need this when configuring Apple Business Manager, so make sure navigate to **Providers** -> **Apple Business Manager SSF** and copy the _SSF Config URL_ value to your text editor.
...
Let's recap what you've done to configured authentik to act as an identity provider for Apple Business Manager.
...
:::info{title="Consequences of locking a domain"}
...
Now that our authentication domain is verified, you're ready to configure federated authentication.
...
You'll be asked how you'd prefer our users sign in. Choose **Custom Identity Provider** and press **Continue**. From the "Set up your Custom Identity Provider" page, you'll need to provide Apple with some information gathered from authentik:
