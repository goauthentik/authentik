...
Additional notes:  If you get a password error when trying to sign on via OpenID, you may need to log in via password and create the admin that matches an admin in Authentik and manually transfer ownership via User Access Management.  Make sure that the admin is set as "admin" in role.  After the transfer of ownership, you may have to reset the role if it doesn't appear in the User Directory (it may be blank).
:::
