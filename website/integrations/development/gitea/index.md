...
2. Navigate to **Customization** > **Property Mappings** and click **Create**. Create a **Scope Mapping** with the following configurations:
    ...
    if if request.user.groups.filter(name="gituser").exists():
        gitea_claims["gitea"]= "user"
    if if request.user.groups.filter(name="gitadmin").exists():
        gitea_claims["gitea"]= "admin"
    if if request.user.groups.filter(name="gitrestricted").exists():
