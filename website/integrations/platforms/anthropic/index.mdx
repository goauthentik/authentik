...
Use JIT group mappings only when Anthropic should assign roles or seat types from authentik. In this guide, the SAML `groups` attribute is populated from authentik application entitlements, so create the entitlements in authentik first, then map those entitlement names in Anthropic.
...
3. Map each IdP group value from the SAML `groups` attribute to the appropriate Anthropic role or seat type.
