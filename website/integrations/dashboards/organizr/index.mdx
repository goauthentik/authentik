...
Organizr supports native OpenID Connect (OIDC) login and proxy authentication. Proxy authentication requires an authentik proxy outpost and LDAP outpost.
...
2. Navigate to **Settings** > **System Settings** > **Main** > **OIDC Global**.
3. Configure the following settings:
    - **Enable OIDC Authentication**: enable this setting.
    - **Auto-Redirect to OIDC** _(optional)_: enable this setting to send users directly to authentik from the Organizr login page.
    - **Auto-Redirect Provider**: select **Authentik** if you enabled automatic redirects.
    - **Auto-Create Users**: enable this setting to create an Organizr account when a user first logs in through authentik.
    - **Link Existing Users**: enable this setting to link existing Organizr accounts by email address.
    - **Default Group**: select the Organizr group for new OIDC users.
4. Navigate to **Settings** > **System Settings** > **Main** > **OIDC: Authentik**.
...
6. Click **Test** to verify that Organizr can retrieve the authentik OpenID Connect configuration.
7. Save your changes.

### Configuration verification

To confirm that authentik is properly configured with Organizr, open the Organizr login page and select **Login with Authentik**. You should be redirected to authentik and returned to Organizr after authentication.
...
:::warning Protect the Organizr backend
When Organizr is configured for proxy authentication, Organizr trusts the proxy headers that it receives. Make sure users can access Organizr only through the authentik proxy outpost, and do not expose the Organizr backend directly to the internet.
:::
...
- [Organizr OIDC Discussion](https://github.com/causefx/Organizr/issues/2070)
