...
3. Navigate to **Login Methods**, click **Configure** next to **SAML**, then click **Add SAML**.
    ...
    - **Identity Provider (IdP) Initiated Login**: toggle as desired.
4. Click **Save**.

:::info
Enabling IdP-initiated login allows users to log in to Datadog directly from the authentik User interface.
:::

## Configuration verification

To confirm that authentik is properly configured with Datadog, log out, then navigate to the Datadog login page and click **Using Single Sign-On?**. Enter your email address and click **Next**. You should be redirected to authentik for authentication. After logging in, you will be redirected back to Datadog.
