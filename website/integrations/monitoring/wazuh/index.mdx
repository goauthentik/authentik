...
6. Edit the `/etc/wazuh-indexer/opensearch-security/roles_mapping.yml` file and include `wazuh-admin` as a `backend_role` in the appropriate section. The following example adds the role to the `all_access` section:
...
4. On the Wazuh Dashboard server, add these lines to the `/etc/wazuh-dashboard/opensearch_dashboards.yml` file. If the entry `opensearch_security.session.keepalive` already exists in your config, simply change it to `false`
    ...
    If you want to use both authentication methods (username/password and SSO) use this config:
