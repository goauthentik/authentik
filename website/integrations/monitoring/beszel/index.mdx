...
Beszel requires the email scope to return a value of `email_verified: True`. As of [authentik 2025.10](/docs/releases/2025/v2025.10.mdx#default-oauth-scope-mappings), the default behavior is to return `email_verified: False`, so a custom scope mapping is required for Beszel to allow authentication.
...
Beszel uses PocketBase as its server backend. When you install Beszel, PocketBase is included as part of Beszel, so you do not need to separately [integrate PocketBase](../../platforms/pocketbase/index.mdx).
:::
