...
### Configure LDAPS
LDAPS is preferred over LDAP because LDAP sends plaintext user passwords over the network.

1. Navigate to **System** > **Certificates** and click **Generate Certificate-Key Pair**
2. Give the certificate a **Common Name** like "LDAPS Certificate for Cisco Modeling Labs"
3. Set a validity period
4. Set the **Algorithm** to **RSA**
5. Click **Generate Certificate-Key Pair**
6. Edit your LDAP Provider, and select your generated certificate in the **Certificate** section.
7. **Save** your changes
...
:::info LDAP Outpost Reuse
If you have an existing LDAP Outpost, you may safely ignore this section and move on to **Cisco Modeling Labs configuration**.
:::
...
:::
:::info Allowing multiple groups for users and admins
To allow multiple groups to log in to Cisco Modeling Labs, append another section `(memberOf=cn=samplegroup,ou=groups,<base_dn>)` to the user filter. Please keep in mind that when adding new groups to the admin filter, the user filter has to reflect the same changes.
:::
...
8. Optionally, assign a **Resource Pool** template. This will clone the Resource Pool for every user registered through LDAP. This is useful for preventing resource abuse on multi-user systems. Refer to CML Documentation on how to create and configure a resource pool.
9. Click **Save**.
