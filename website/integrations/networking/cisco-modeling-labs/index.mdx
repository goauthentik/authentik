...
If you configure application bindings, ensure that `cml-ldap` and the `cml-users` group have access to the LDAP application.
...
To confirm that authentik is properly configured with Cisco Modeling Labs, open the CML UI and log in with an authentik username and password. A user in `cml-users` should reach the CML dashboard, and a user in both access groups should have administrator privileges.
