...
wg-easy only accepts logins where the email scope returns `email_verified: True`. Because the default authentik email scope mapping returns `email_verified: False`, create a custom scope mapping for wg-easy. See [Email scope verification](/docs/add-secure-apps/providers/oauth2/#email-scope-verification).
