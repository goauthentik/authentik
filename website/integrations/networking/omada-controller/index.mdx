...
To support the integration of Omada Controller with authentik, you need to create property mappings, an application entitlement, and an application/provider pair in authentik.
...
5. Click **Finish** to save. Then, repeat the process to create a mapping for the user's Omada role entitlement:
    ...
    yield omada_entitlements[0] if omada_entitlements else None
    ```
...
1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Applications** and click **New Application** to create an application and provider pair. (Alternatively you can first create a provider separately, and then create the application and connect it with the provider.)
...
### Create an application entitlement in authentik

1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Applications** and open the Omada Controller application after you create it.
3. Click the **Application entitlements** tab.
4. Create an entitlement such as `Omada-admins`. The entitlement name must start with `Omada-` for the `omada_usergroup_name` property mapping to function correctly.
5. Open the entitlement and bind the user or group that needs Omada Controller administrator access to it.

:::warning
Omada can't handle a user being in multiple roles. Therefore, ensure that a user receives only a single `Omada-` entitlement.
:::
...
8. At the top-right corner of the page, click **Go to SAML User Group**, and then **Add New SAML User Group**.
9. Set the desired **SAML User Group Name**, **Role**, and **Privileges** for the new SAML user group. The **SAML User Group Name** must exactly match the name of the previously created authentik entitlement. It is case sensitive.
10. Click **Create**.
...
- [Omada Networks Documentation - How to Configure SAML SSO on Omada Controller](https://www.omadanetworks.com/en/support/faq/4406/#_Toc193896083)
