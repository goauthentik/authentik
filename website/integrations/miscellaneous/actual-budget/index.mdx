...
:::warning
Please note that users are not created automatically in Actual Budget. The owner must manually provision users.

To do so, login as a local (non-OIDC) admin, then navigate to **Server online** > **User Directory**, and create users matching existing authentik usernames. Then, grant access to the budget via the **User Access** tab.

If you want to create an admin user with OIDC integration you'll need to provision an account with the `admin` role.  To also configure this admin account as an owner, you can transfer ownership via **User Access Management**.  After the transfer of ownership, you may need to reset the `admin` role if it doesn't appear in the **User Directory**.
:::

:::warning
If you do not provision user accounts before OIDC authentication, you will receive a password error.
:::
