...
1. Login as a local (non-OIDC) admin
...
For admin users with OIDC integration, provision an account with the `admin` role. To configure this admin as an owner, transfer ownership via **User Access Management**. You may need to reset the `admin` role afterward if it doesn't appear in the **User Directory**.
:::
