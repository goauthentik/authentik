...
1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Applications** and click **Create** and provide the following required confiurations.
...
To confirm that authentik is properly configured with Amazon Business, log out of Amazon Business. On the Amazon Business portal select **Single Sign-On** (or **Sign in with SSO**) and enter an email address that's configured for SSO. You should be redirected to authentik to login, and if successful, you should then be redirected to the Amazon Business portal.
