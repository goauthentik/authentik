...
1. Sign in to the Amazon Business admin console as an administrator and click **Buisness Settings** under **Hello, "Your Name"**.
2. Under **System integrations**, select **Single Sign-On (SSO)**. You will be redirected to the Amazon Business SSO setup workflow (for example, `https://www.amazon.de/ab/sso/setup/edit/select-idp`; your domain may use a different TLD such as `.com` or `.co.uk`).
...
5. Under **Advanced protocol settings**, adjust the following settings:
...
1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Applications** and click **Create** and provide the following required configurations.
    - Provide a name and slug.
    - For **Provider**, select the previously created provider.
3. Click **Create**.

### Download the metadata file

1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Providers** and click on the name of the newly created Amazon Business provider.
...
To confirm that authentik is properly configured with Amazon Business, log out of Amazon Business. On the Amazon login page, simply enter your business email address and click **Next**. You should be redirected to authentik to log in, and if successful, redirected to the Amazon Buisness portal.
