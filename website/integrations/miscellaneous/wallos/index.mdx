...
If existing Wallos users should be linked by email address, Wallos requires the `email_verified` claim to be `True`. Refer to [Email scope verification](/docs/add-secure-apps/providers/oauth2/#email-scope-verification) for instructions on how to create a custom email scope mapping that returns the required claim.
