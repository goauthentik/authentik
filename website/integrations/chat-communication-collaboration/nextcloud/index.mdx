    ...
    - **Create Scope Mapping**:
        ...
        - **Expression**:
            ...
            return {
                ...
                "groups": groups,
                "quota": quota,
                "user_id": request.user.attributes.get("nextcloud_user_id", request.user.username.lower()),
            }
            ```

3. Click **Finish**.

To set a quota, define the `nextcloud_quota` attribute on a user, group, or Nextcloud application entitlement. For example, setting it to `1 GB` restricts the user to 1 GB of storage. To connect an authentik user to an existing Nextcloud account, set the user's `nextcloud_user_id` attribute to the existing Nextcloud username. If the authentik username already matches the existing Nextcloud username, the `nextcloud_user_id` attribute can be omitted.
...
2. Navigate to **Applications** > **Applications** and click **New Application** to open the application wizard.
    ...
    - **Configure the Provider**: provide a name (or accept the auto-provided name), the authorization flow to use for this provider, and the following required configurations.
        ...
        - Set the **SLS URL** to `https://nextcloud.company/apps/user_saml/saml/sls`.
