    ...
    - **Create Scope Mapping**:
        ...
        - **Expression**:
            ...
            return {
                ...
                "user_id": user.attributes.get("nextcloud_user_id", str(user.username.lower())),
            }
            ```
...
:::info
To connect to an existing Nextcloud user, set the `nextcloud_user_id` attribute to match the Nextcloud username (found under the user's `Display name` in Nextcloud).  If the Authentik username matches the Nextcloud username, you can use `user.username.lower()`.
:::
