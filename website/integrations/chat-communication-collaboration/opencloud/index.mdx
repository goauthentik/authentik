...
Create one **Application + OAuth2/OpenID Connect provider** pair per client. On **every** provider set the same values:

- **Client type**: `Public`
- **Signing Key**: the **same** key for all providers (the shared issuer exposes a single `jwks_uri`, so all clients' tokens must be signed by one key).
- **Issuer mode** (under Advanced protocol settings): `Same identifier is used for all providers`.
- **Scopes**: `openid`, `profile`, `email`, and `offline_access` (the latter is required for the desktop/mobile sync clients to receive a refresh token).

Then set the per-client **Client ID** and **Redirect URIs**:
...
Use the same overlay and role override as the **Web only** tab, but point `OC_OIDC_ISSUER` at the shared (root) issuer and give each platform its WebFinger client ID.
