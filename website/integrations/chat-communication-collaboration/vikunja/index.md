...
:::info
This is based on authentik 2025.12.4 and Vikunja V1.1.0 using the Docker Compose install https://vikunja.io/docs/full-docker-example/. Instructions may differ between versions. Vikunja uses different OpenID configuration syntax for versions before 1.0.
:::
...
- `authentik` is the provider key set in `config.yml` under `auth.openid.providers`. This key is used as the path segment in the redirect URI (e.g., `/auth/openid/authentik`). If the key contains uppercase letters or spaces, they are converted to lowercase with spaces removed in the callback URL.
...
- **Configure the Provider**: provide a name (or accept the auto-provided name), the authorization flow to use for this provider, and the following required configurations.
    ...
    - Set a `Strict` redirect URI to `https://vik.company/auth/openid/authentik`.
...
auth:
    ...
    openid:
        ...
        providers:
            # The ID of the provider as it will appear in the frontend. Must match the redirect URI
            authentik:
                # Internal/friendly name. Can be anything
