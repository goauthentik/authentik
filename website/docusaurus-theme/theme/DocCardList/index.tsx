...
function getStableKey(item: GlossaryItem | PropSidebarItem, idx: number) {
    if (typeof item === "object" && item !== null) {
        if (hasOwnProperty(item, "docId") && typeof item.docId === "string") return item.docId;
        if (hasOwnProperty(item, "id") && typeof item.id === "string") return item.id;
        if (hasOwnProperty(item, "href") && typeof item.href === "string") return item.href;
        if (hasOwnProperty(item, "label") && typeof item.label === "string") return item.label;
    }
    return idx;
}
