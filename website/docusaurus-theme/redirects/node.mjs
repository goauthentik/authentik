 ...
 *
 * 1. `redirects/plugin` exposes the parsed entries as global data, so the client-side router can
 *    follow redirects when navigating to a missing page (see `theme/NotFound`).
 * 2. `@docusaurus/plugin-client-redirects` creates build-time redirect pages for HTTP navigation.
 *    Netlify's deployment additionally picks up the `_redirects` file itself, covering sources no
 *    longer present in the build output, such as removed, renamed, or moved pages.
 *
 * @param {string} redirectsFile Absolute path to a Netlify `_redirects` file
 * @param {Omit<ClientRedirectsOptions, "createRedirects">} [clientRedirectsOptions] Extra options
 *   for `@docusaurus/plugin-client-redirects`, e.g. static `redirects`
 *
