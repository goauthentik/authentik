...
Use the complete output from the target authentik version's [`hash_password` command](../core/password-hashes.mdx#generate-a-password-hash). authentik validates the value against the [password hash import settings](../core/password-hashes.mdx#password-hash-import-settings). If validation fails, authentik stops initial setup with a configuration error. Bootstrap imports do not support an override.
