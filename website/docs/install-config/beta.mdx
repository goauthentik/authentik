...
- **Beta versions**: Early access to major new features that are still in active development. Beta versions may have more bugs and breaking changes.

It is recommended to upgrade to the latest stable release before upgrading to RC or Beta images.

:::warning
Downgrading from RC or Beta versions is not supported. It is recommended to take a backup before upgrading, or test these versions on a separate install. Upgrading from RC or Beta versions to the next stable release is usually possible, however also not supported.
:::
...
Add the following block to your `values.yml` file, replacing `2025.10.0-rc3` with the desired RC version:
