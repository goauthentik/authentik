...
- [Upgrade PostgreSQL on Kubernetes](../troubleshooting/postgres/upgrade_kubernetes.md)

### Upgrade sequence

Upgrades must follow the sequence of major releases and utilise the most recent minor release; **do not skip** directly from an older major version to the most recent version.

For example, if you're currently running `2025.2.1`, upgrade in the following order:

1. Upgrade to the latest `2025.2.x`.
2. Then to the latest `2025.4.x`.
3. Finally to the latest `2025.6.x`.

Always use the latest patch version (`.x`) within each `major.minor` version before proceeding to the next major version.

### Outposts

The version of the authentik server and all authentik outposts must match. Ensure that all [outposts are upgraded](../add-secure-apps/outposts/upgrading.md) at the same time as the core authentik instance.
