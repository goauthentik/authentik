...
authentik only accepts forwarded headers, including `X-Forwarded-Proto` and `X-Forwarded-For`, when the request comes from a trusted proxy network. authentik does not use forwarded headers from other sources to determine the original host, scheme, or client address.
