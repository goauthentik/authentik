...
- `AUTHENTIK_POSTGRESQL__CONN_OPTIONS`
    ...
    Arbirarty `libpq` keyword parameters for the database connection. A list of options can be found [in the PostgreSQL documentation](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-PARAMKEYWORDS).
    ...
    The format is a base64-encoded JSON dictionary.
...
Additionally, arbitrary connection parameters can be set on all read replicas with:

- `AUTHENTIK_POSTGRESQL__REPLICA_CONN_OPTIONS`

    Arbirarty `libpq` keyword parameters for all read replicas database connections.
