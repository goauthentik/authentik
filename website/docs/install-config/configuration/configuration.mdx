...
- `AUTHENTIK_POSTGRESQL__HOST`: Defines the hostname or IP address of your PostgreSQL server where authentik will store its data.
- `AUTHENTIK_POSTGRESQL__NAME`: Specifies the name of the database that authentik will use within PostgreSQL.
- `AUTHENTIK_POSTGRESQL__USER`: Defines the username that authentik will use to authenticate with PostgreSQL.
- `AUTHENTIK_POSTGRESQL__PORT`: Specifies the port on which PostgreSQL is listening. Defaults to the standard PostgreSQL port `5432`.
- `AUTHENTIK_POSTGRESQL__PASSWORD`: Defines the password that authentik will use to authenticate with PostgreSQL. If not specified, defaults to the value of the environment variable `POSTGRES_PASSWORD`.
...
- `AUTHENTIK_POSTGRESQL__DEFAULT_SCHEMA`: Defines the name of the schema used by default in the database. Defaults to `public`. :ak-version[2024.12]
...
- `AUTHENTIK_POSTGRESQL__SSLROOTCERT`: Specifies the file path to the Certificate Authority (CA) certificate used to verify the server's SSL certificate. Required when using `verify-ca` or `verify-full` modes.
...
- `AUTHENTIK_POSTGRESQL__SSLKEY`: Specifies the file path to the private key corresponding to the client certificate specified in `SSLCERT`. Used to prove authentik's identity to the PostgreSQL server.

### Connection Management

- `AUTHENTIK_POSTGRESQL__CONN_MAX_AGE`: Controls how long database connections are kept open in seconds. Defaults to `0` (connections are closed after each request). Possible values:

    - `0`: No persistent connections.
    - Any positive integer: Number of seconds to keep connections open.
    - `null`: Unlimited persistent connections.

    See [Django's documentation](https://docs.djangoproject.com/en/stable/ref/settings/#conn-max-age) for more details on how this affects performance and resource usage.

- `AUTHENTIK_POSTGRESQL__CONN_HEALTH_CHECKS`: Enables health checks on persistent database connections before they are reused. Helps prevent the use of stale connections that might have been closed by the server. Defaults to `false`. Possible values:

    - `true`: Perform health checks.
    - `false`: Do not perform health checks.
...
- `AUTHENTIK_POSTGRESQL__DISABLE_SERVER_SIDE_CURSORS`: Disables server-side cursors when set to `true`. Server-side cursors can improve performance for large result sets but may not be compatible with all database configurations. Defaults to `false`. Possible values:

    - `true`: Disable server-side cursors.
    - `false`: Enable server-side cursors.

    See [Django's documentation](https://docs.djangoproject.com/en/stable/ref/settings/#disable-server-side-cursors) for more details on when you might want to disable server-side cursors.
...
You can configure additional read replica databases to distribute database load and improve performance. If read replicas are configured, the main database is not used for reads. If you'd like the main database to be included for reads, add it as a read replica as well.
...
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__HOST`: The hostname or IP address of the read replica server.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__NAME`: The database name on the read replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__USER`: The username for authenticating with the read replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__PORT`: The port on which the read replica is listening.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__PASSWORD`: The password for authenticating with the read replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLMODE`: The SSL verification mode for the replica connection.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLROOTCERT`: The path to the CA certificate for verifying the replica's SSL certificate.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLCERT`: The path to the client certificate for authenticating with the replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLKEY`: The path to the client private key for authenticating with the replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__CONN_MAX_AGE`: The connection lifetime for connections to the replica.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__CONN_HEALTH_CHECKS`: Whether to perform health checks on persistent replica connections.
- `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__DISABLE_SERVER_SIDE_CURSORS`: Whether to disable server-side cursors for the replica.

### Using a PostgreSQL Connection Pooler

When your PostgreSQL database(s) are running behind a connection pooler (like PgBouncer or PgPool), you need to adjust several settings to ensure compatibility:
