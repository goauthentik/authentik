...
:::warning
If you are running a [development environment](../../developer-docs/setup/full-dev-environment.md) and you have read replicas configured in your `.env` file, you might encounter problems logging back in. This is because dev environmentsdo not have read replica DBS, but the `.env` file is configured to call them. If you cannot log in, edit your local `.env` file to remove the postgresql.read_replicas entry.
:::
