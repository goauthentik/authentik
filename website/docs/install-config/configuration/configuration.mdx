...
- `AUTHENTIK_POSTGRESQL__CONN_OPTS__<key>`

    Allows the user to pass arbirarty libpq keyword parameters to the database connection. A list of options can be found [in the postgresql documentation](https://www.postgresql.org/docs/current/libpq-connect.html#LIBPQ-PARAMKEYWORDS)

    Options passed via `CONN_OPTS` will overwrite those passed in the default manner. For example `AUTHENTIK_POSTGRESQL__CONN_OPTS__SSLMODE` will overwrite the default `AUTHENTIK_POSTGRESQL__SSLMODE` field.

    These custom options will not be applied to any read replica servers.
...
Arbitrary connection options can be passed to read replicas in two ways.
- `AUTHENTIK_POSTGRESQL__GLOBAL_REPLICA_CONN_OPTS__<key>`

    This will apply the same options to all read replica instances. These global options are applied before the default options or any replica specific options are parsed therefore `AUTHENTIK_POSTGRESQL__GLOBAL_REPLICA_CONN_OPTS__SSLMODE` will be overwritten by `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLMODE` but only for that replica.

- `AUTHENTIK_POSTGRESQL__READ_REPLICA__0__CONN_OPTS__<key>`

    Options defined this way will only be applied to that replica. Similarly to the primary connection options, these are applied last and will overwrite the defaults. (e.g. `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__SSLMODE` will be overwritten by `AUTHENTIK_POSTGRESQL__READ_REPLICAS__0__CONN_OPTS__SSLMODE`)
