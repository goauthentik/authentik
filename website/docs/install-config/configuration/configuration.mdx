...
These settings define the default values for account recovery. If the Email Stage specifies its own values, those will take precedence.

- `AUTHENTIK_EMAIL_RECOVERY__MAX_ATTEMPTS`: Maximum number of account recovery attempts allowed before rate limiting is applied.
- `AUTHENTIK_EMAIL_RECOVERY__CACHE_TIMEOUT`: Duration (in seconds) before the rate limiting cache expires.
