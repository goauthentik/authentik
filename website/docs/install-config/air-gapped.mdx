...
On a system with internet access, download the target release's `compose.yml`, then mirror the authentik server image referenced by that file. If you use managed outposts, mirror the matching outpost images as well.
