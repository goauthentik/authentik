...
The [event map](../sys-mgmt/events/index.md#event-map) uses a bundled basemap and does not connect to an external tile service by default. If you configure a custom basemap under **System** > **Brands**, users' browsers must be able to access the tile source. In an air-gapped environment, use an internally available tile source or leave **Map tiles** empty to use the bundled basemap.
