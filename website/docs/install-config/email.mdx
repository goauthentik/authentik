...
To test the email settings of a specific email stage, you can utilize the `-S` parameter:
...
If you're configuring authentik to send email via an SMTP server that has TLS enabled...
...
Add the following to your Docker compose configuration so that your authentik container trusts the same root certificates as the docker host:
...
Or if the certificate is located elsewhere:
...
Add the following to your Kubernetes `values.yaml` file
