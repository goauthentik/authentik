...
Email stages can also use stage-specific SMTP settings when a flow must send through a different mail server than the server used by other functions in authentik.
