...
It is recommended to configure global email settings. These are used by authentik to notify you about alerts and configuration issues. They can also be used by [Email stages](../add-secure-apps/flows-stages/stages/email/index.mdx) to send verification/recovery emails.
...
To configure email settings, append this block to your `.env` file
...
Or, if you are using the Kubernetes Helm chart, set the following variables in the `email` section:
