...
<<<<<<< HEAD:website/docs/install-config/reverse-proxy.md
...
- CSRF errors when saving objects are usually caused by incorrect `Host` or `Origin` handling. See [Troubleshooting CSRF Errors](../troubleshooting/csrf.mdx).
>>>>>>> ed934ee31 (website/docs: trusted proxy addresses change (#25637)):website/docs/install-config/reverse-proxy.mdx
