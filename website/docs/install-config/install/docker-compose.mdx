...
:::info
If you are running on Podman and want first-class systemd integration, see the [Podman with Quadlet installation](./podman-quadlet.mdx) guide instead — it deploys authentik as native `.service` units without `podman compose`.
:::
