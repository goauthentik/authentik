...
- **Configure the Provider**:
    ...
    - **Protocol settings**: provide the following required configurations:
        ...
        - Set the **Redirect URI**as a `Strict` redirect to `https://grafana.company/login/generic_oauth`.
            - <strong className="tip">TIP</strong>: The Redirect URI is where where a user is directed to, as soon as authentik's authorization flow is successfully completed.
    ...
    - **Logout Method**: set to `Front-channel`.
        - <strong className="tip">TIP</strong>: With OAuth2, front-channel logout is considered the default because most applications (including Grafana) do not support back-channel logout.
