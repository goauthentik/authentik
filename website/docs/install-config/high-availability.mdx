...
- Multiple instances of the server and worker containers can be run in parallel, allowing for redundancy.
...
If a server or worker container goes offline, another container can continue to provide service availability. If these containers are distributed among multiple hosts, it can provide even higher levels of resilience and fault tolerance.
...
authentik also has built-in support for [PostgreSQL Read Replicas] and using [PostgreSQL Connection Poolers](https://docs.goauthentik.io/install-config/configuration/#using-a-postgresql-connection-pooler) like PgBouncer or PgPool.
