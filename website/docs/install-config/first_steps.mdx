...
If your attempt to open authentik in the browser using the URL with the `initial-setup` flow does not work, verify that you have a trailing slash on the end of the URL. Also verify that Docker is running and that the authentik server and worker are started and healthy.
:::
