...
Now that you have your authentik instance installed and configured with the required settings, you can add your first [application](../core/glossary/terms/application.mdx) and [provider](../core/glossary/terms/provider.mdx). After that, we'll walk through how to add users, groups, and roles.
...
**B.** Provide the details for the application and provider.
...
**A.** In your `.env` file, set the following environment variables (these are for a [Grafana instance running in Docker](https://grafana.com/docs/grafana/latest/setup-grafana/configure-docker/)):
...
**B.** Save your `.env` file and go to your Grafana application.
...
Now that you can access the authentik Admin interface, and you have added an application and provider, let's add a new users and assign them appropriate groups and roles.
