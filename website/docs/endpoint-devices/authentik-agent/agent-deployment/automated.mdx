...
After the authentik Agent is deployed to a device, the user receives a notification asking them to register with authentik. The registration flow prompts the user to authenticate. After authentication, authentik enrolls the device and associates it with the user.
