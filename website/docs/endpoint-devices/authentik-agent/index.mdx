...
| **Linux only**            | libnss-authentik                  | NSS Module that makes Linux aware of authentik users. All authentik users will be visible to Linux - but won't be able to login unless configured via device access groups. Provides a consistent `uid` and `guid` for users on all Endpoint Devices. | authentik-sysd, libpam-authentik |
...
All authentik Agent components communicate via GRPC and system sockets. (TODO)
