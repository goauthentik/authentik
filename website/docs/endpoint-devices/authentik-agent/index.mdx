...
PAM Module which provides token-based and interactive authentication via authentik. Requires authentik-sysd to be running.
...
NSS Module that makes the Linux device aware of authetnik users. Similar to adding a Linux device to an Active Directory or LDAP domain.Requires authentik-sysd to be running.
