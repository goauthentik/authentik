...
You can use the [authentik agent](../authentik-agent/index.mdx) to authenticate SSH connections using authentik credentials between endpoint devices.
...
- [authentik Agent needs to be deployed](../authentik-agent/agent-deployment/index.mdx) on both endpoint devices; the source and the SSH target.
