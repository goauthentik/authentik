...
- You must have [configured compliance](./configuration.mdx) in authentik
- The [Fleet connector](./connectors/fleetdm.mdx) must be configured in authentik
...
The flow must have an [Endpoint stage](../../add-secure-apps/flows-stages/stages/endpoint/index.mdx) bound to it.
