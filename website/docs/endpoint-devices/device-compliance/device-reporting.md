...
These facts are shown on the [Devices Overview](../devices-overview.mdx) page and are also accessible to policies like the [Device Compliance Policy](./device-compliance-policy.md) and can be used to make authenticationa and authorization decisions. For example, you can create a policy that only allows Endpoint Devices that are running a recent OS version to access an application.
