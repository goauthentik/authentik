...
3. Select **Google Device Trust Connector** as the connector type, click **Next**, and configure the following settings:
    ...
    - **Google Verified Access API**
        - **Credentials**: create or select a secret containing the JSON key file that you downloaded earlier. Use the **File** type to upload it, or **Multi-line text** to paste its contents.
