...
The Agent connector requires a **Challenge Key** (certificate keypair) to be configured when using the [Endpoint Stage](../../../add-secure-apps/flows-stages/stages/endpoint/index.mdx). This keypair is used to sign challenges sent to the [browser extension](../browser-extension.mdx) for device verification.
...
After creating the connector, it can be used in the [Endpoint Stage](../../../add-secure-apps/flows-stages/stages/endpoint/index.mdx). Refer to [Device compliance policy](../device-compliance-policy.mdx) for more information on using device facts from the connector in a flow.
