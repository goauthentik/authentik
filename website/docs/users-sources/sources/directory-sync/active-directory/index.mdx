...
| **Enable StartTLS**              | Enabled                         | Upgrades the connection to TLS before binding. To use LDAPS instead, use `ldaps://` and disable this setting. Both options support AD password writeback.                                |
...
Choose how authentik represents nested Active Directory groups:

- [Preserve the Active Directory group hierarchy](#preserve-the-active-directory-group-hierarchy) to keep direct and inherited memberships distinct.
- [Let Active Directory resolve recursive membership](#let-active-directory-resolve-recursive-membership) to store effective memberships as direct memberships in authentik.
