...
- `authentik.company` is the FQDN of the authentik installation.
- `<scim-slug>` is the slug of the SCIM source you create in authentik.
...
3. Provide a **Name** and **Slug** (for example, `jumpcloud`), then click **Finish**.
4. Open the newly created source. Note the **SCIM Base URL** (`https://authentik.company/source/scim/<scim-slug>/v2`), then use **Click to copy token** to copy the bearer token. Both are required in the next section.
...
5. Open the **Provisioning** tab and configure the following settings:
    - **Base URL**: `https://authentik.company/source/scim/<scim-slug>/v2`
