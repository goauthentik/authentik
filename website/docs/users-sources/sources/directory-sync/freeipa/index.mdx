...
To integrate authentik with FreeIPA, create an LDAP source in authentik.
...
| **Use Server URI for SNI verification** | Enabled                                                        | Matches the blueprint below.                                                                                                                             |
...
| **Bind CN**                             | `uid=svc_authentik,cn=users,cn=accounts,dc=freeipa,dc=company` | Enter the service account's user principal name (UPN).                                                                                                   |
| **Bind Password**                       | The service account password                                   | —                                                                                                                                                        |
| **Base DN**                             | `dc=freeipa,dc=company`                                        | Defines the search base.                                                                                                                                 |

:::tip Multiple server URIs
You can specify multiple server URIs separated by commas. For example, specify `ldap://ipa1.freeipa.company,ldap://ipa2.freeipa.company`. If you use a DNS record with multiple entries, authentik picks one at random on the first connection.
:::
...
authentik can represent nested FreeIPA groups in two ways. It can synchronize direct user and group relationships to reproduce the FreeIPA group hierarchy, or it can use the effective user memberships resolved by FreeIPA. Both approaches can provide effective memberships, but they create different relationships in authentik. Hierarchy synchronization preserves the distinction between direct and inherited membership, while FreeIPA-resolved results are stored as direct memberships.
...
| ---------------------------------------- | ------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| Let FreeIPA resolve effective membership | You need effective membership but not the FreeIPA group structure. | Direct and indirect memberships become direct memberships in authentik. |
| Preserve the FreeIPA group hierarchy     | You need authentik to reproduce nested FreeIPA groups.             | Direct and inherited memberships remain distinct.                       |
...
You can also configure the LDAP source with a blueprint. This example enables password writeback and deletion of objects no longer found. Complete the writeback preparation in [FreeIPA setup](#freeipa-setup), or set `sync_users_password: false` if you do not want writeback. Internal password updates on LDAP login remain disabled by default.

:::info
Set the bind account's full DN and password in the `FREEIPA_DN` and `FREEIPA_PASSWORD` environment variables. Ensure that the variables are available to authentik when it applies the blueprint. Before applying the blueprint, import the trusted FreeIPA CA chain into authentik and set the imported certificate's UUID as the `peer_certificate` value under `attrs`. Omitting this reference skips certificate validation.
:::
