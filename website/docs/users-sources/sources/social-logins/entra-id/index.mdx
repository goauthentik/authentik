...
To support the integration of Entra ID with authentik, you need to create an application/provider pair in authentik. Optionally, if you need to read group membership information from Entra ID you will also need to create a property mapping.

### Create OAuth source property mappings \_(optional)
...
2. Navigate to **Directory** > **Federation and Social login** and click **Create**.
    ...
    - **Create Entra ID OAuth Source**: provide a name, a slug which must match the slug used in the Entra ID `Redirect URI`, and the following required configurations.
        - Under **Protocol Settings**:
            ...
            - **Scopes**_(optional)_: if you need to read group membership information from Entra ID, add the `https://graph.microsoft.com/GroupMember.Read.All` scope.
        - Under **URL Settings**:
            ...
            - **OIDC Well-known URL**: if the default `Single tenant` account type was selected in Entra ID, then you must change this URL to `https://login.microsoftonline.com/<Directory_(tenant)_ID>/v2.0/.well-known/openid-configuration`
        - Under **OAuth Attribute Mapping**:
            ...
            - **Group Property Mappings**_(optional)_: if you need to read group membership information from Entra ID, select the `Entra ID Group Property Mapping` created in the previous section.
