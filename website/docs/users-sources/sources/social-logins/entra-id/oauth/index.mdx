...
This source lets users authenticate to authentik with their Entra ID credentials by configuring Entra ID as a federated identity provider with OAuth 2.0.
...
2. Navigate to **Directory** > **Federation and Social login**, click **New Source**, and then configure the following settings:
    ...
    - **Create Entra ID OAuth Source**: provide a name, a slug that must match the slug used in the Entra ID `Redirect URI`, and the following required configurations:
...
When group membership information is synced from Entra ID, authentik creates the groups returned by Microsoft Graph's `/me/memberOf` response. The OAuth source currently reads only the first page of that response, so a user with more memberships than the page contains will have incomplete group sync. The sync runs on each user login, which can also delay logins for organizations with many groups.
...
The JWT returned from the request above can be used in authentik and exchanged for an [authentik JWT](../../../../../add-secure-apps/providers/oauth2/machine_to_machine.mdx#authentik-issued-jwts).

## Source property mappings

The Entra ID OAuth source already maps the Microsoft Graph `/me` response to the user's username, email, and name. If you enable the `GroupMember.Read.All` permission and scope above, it also retrieves the user's direct group memberships from `/me/memberOf` and synchronizes those groups on login. You do not need a custom mapping for these fields.

Use an OAuth source property mapping for additional fields in the profile response. For example, this user mapping stores the Graph `jobTitle` field as a user attribute:

```python
return {"attributes": {"job_title": info.get("jobTitle")}}
```

To add it, open **Customization** > **Property Mappings**, create an **OAuth Source Property Mapping** with the expression above, and then edit the Entra ID OAuth source. Under **OAuth Attribute mapping** > **User Property Mappings**, add it to **Selected User Property Mappings** and click **Update**.

The source uses each Graph group ID as the stable group identifier and the group's `displayName` as its name. A group property mapping can use `group_id` and the corresponding entry in `info["raw_groups"]` to store more group data. For example:

```python
group = info.get("raw_groups", {}).get(group_id, {})
return {"attributes": {"entra_id": group_id, "description": group.get("description")}}
```

Add that mapping under the source's **Group Property Mappings**. It runs only for groups returned by the Entra ID source; the Graph group permission and scope are still required. See [source property mappings](../../../property-mappings/index.mdx#group-synchronization) and [OAuth expression data](../../../protocols/oauth/index.mdx#expression-data) for details.
