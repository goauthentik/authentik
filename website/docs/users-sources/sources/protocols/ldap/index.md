...
- **Membership field**: The user object attribute or the group object attribute that determines the group membership for an object. If **Lookup using a member attribute** is set, this should be a member object attribute (`memberOf`), otherwise a group object attribute (`member`).
