...
When the IdP sends a `transient` NameID, authentik creates a temporary user linked to the SAML source. This is determined by the NameID format in the assertion, even if the source's **NameID Policy** requests another format.

Temporary users are deleted:

- when they log out of authentik, or
- by a cleanup task after they expire.

The **Delete temporary users after** setting determines the expiration time from account creation. The default is `days=1`; you can set values such as `hours=8` or `hours=1;minutes=30`. This setting does not delay deletion on logout. Cleanup tasks run periodically, rather than at the exact expiration time, and do not check for active sessions, so an expired temporary user can be deleted while still logged in.

Mapping a transient user's username or email to a stable value does not make the account persistent or prevent deletion.

Use **Persistent** NameIDs when users need to keep the same authentik account across logins. Configure the IdP to send a stable persistent NameID and set the source's **NameID Policy** to **Persistent**. authentik uses the NameID to find the existing source connection on subsequent logins.
