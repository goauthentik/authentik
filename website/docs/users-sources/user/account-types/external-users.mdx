...
For information on creating and managing external users, see [Managing users](../user_basic_operations.mdx).
...
Configure a default application for [brands](../../../customize/branding/index.mdx#external-user-settings) used by external users. Without a default application, an external user who signs in without requesting a specific application receives an access-denied page.
...
Use an [internal user](./internal-users.mdx) when someone needs access to the application dashboard to launch multiple applications or manage their user settings from the user interface.
