...
For an overview of all account types, see [Account types](./index.mdx). For information on creating and managing internal users, see [Managing users](../user_basic_operations.mdx).
...
Use an [external user](./external-users.mdx) when the user typically authenticates to a single application and does not need the application dashboard.
