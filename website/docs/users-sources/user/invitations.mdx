...
- **Custom attributes**: (_optional_) YAML or JSON that is added to the flow's `prompt_data`. The data can set built-in user properties, such as `name` and `email`. It can also set custom user attributes under `attributes`. A key only needs to match a field in the flow's [prompt stage](../../add-secure-apps/flows-stages/stages/prompt/index.mdx) if you want the value to appear in that field. See the [example custom attributes](#step-3-create-the-invitation-object) below.
...
<summary>Example custom attributes:</summary>
        ...
        **Set built-in properties and custom user attributes (JSON)**:
        ...
        You can also use dotted keys that match prompt fields:
    ...
    authentik adds invitation data to the flow's `prompt_data`. A later [User Write stage](../../add-secure-apps/flows-stages/stages/user_write/index.mdx) can store keys such as `email` and `name`. It can also store an `attributes` dictionary or dotted keys such as `attributes.department`. A key only needs to match a prompt field if you want the value to appear in that field.
    :::
...
- The enrollment flow does not run a User Write stage after the invitation and prompt stages.
- A custom value uses a key such as `department` instead of an `attributes` dictionary or a dotted key such as `attributes.department`.
- The value is displayed by a prompt field whose **Field Key** does not start with `attributes.`.

### Application receives a default or empty value

If the value appears in the user's attributes but not in the application, review the provider's [property mappings](../../add-secure-apps/providers/property-mappings/index.mdx). Storing an attribute on a user does not automatically send that value through the provider.
