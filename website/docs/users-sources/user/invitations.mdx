...
- **Custom attributes**: (_optional_) YAML or JSON that authentik adds to the flow's `prompt_data`. The data can set built-in user properties, such as `name` and `email`, and custom user attributes under `attributes`. A key only needs to match a field in the flow's [prompt stage](../../add-secure-apps/flows-stages/stages/prompt/index.mdx) if you want the value to appear in that field. See the [example custom attributes](#step-3-create-the-invitation-object) below.
...
- **Custom attributes**: (_optional_) Enter JSON or YAML to add data to the flow's `prompt_data`, for example to pre-fill prompt fields or set custom user attributes.
...
<summary>Example custom attributes:</summary>
        ...
        **Pre-fill multiple fields (YAML)**:
            ...
            ```yaml
            name: Dominic Roy
            email: dominic.roy@company.com
            ```

        **Set built-in properties and custom user attributes (JSON)**:
        ...
        Dotted keys that match prompt fields work as well:
    ...
    :::info How invitation data is used
    authentik adds the invitation's custom attributes to the flow's `prompt_data`. A later [User Write stage](../../add-secure-apps/flows-stages/stages/user_write/index.mdx) stores keys such as `email` and `name` on the user, along with an `attributes` dictionary or dotted keys such as `attributes.department`. A key only needs to match a prompt field if you want the value to appear in that field.
    :::

    For an end-to-end example, see [Collect custom user attributes during enrollment](../../add-secure-apps/flows-stages/flow/collect-user-attributes.mdx).
...
- The invitation stage is not evaluated before prompt stages in the flow.

### Invitation data not stored on the user

Possible causes:

- The enrollment flow does not include a [User Write stage](../../add-secure-apps/flows-stages/stages/user_write/index.mdx), or the stage runs before the Invitation or Prompt stages.
- The custom attributes use a key such as `department` instead of an `attributes` dictionary or a dotted key such as `attributes.department`.

### Application receives a default or empty value

If the value appears in the user's attributes but not in the application, review the provider's [property mappings](../../add-secure-apps/providers/property-mappings/index.mdx). Storing an attribute on a user does not send that value through a provider. For OAuth2/OIDC, also confirm that the scope mapping is assigned to the provider and that the application requests the scope.
