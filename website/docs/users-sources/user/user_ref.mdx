...
- `composite_status`: Overall state of the user, see [Status](#status). Read-only.
...
`composite_status` combines the conditions that stop a user from signing in into a single value, shown in the **Status** column of the user list in the Admin interface. It is derived from the user's other fields rather than stored, so it cannot fall out of step with them.

| Value             | Meaning                                                                                                                                                              |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `active`          | The user can sign in.                                                                                                                                                |
| `password_locked` | The user's password was locked after too many failed attempts. See [Password lockout](../../add-secure-apps/flows-stages/stages/password/index.md#password-lockout). |
| `deactivated`     | The user is not active. This takes precedence over a locked password, because a deactivated user cannot sign in regardless.                                          |
