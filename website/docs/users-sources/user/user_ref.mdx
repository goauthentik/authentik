...
A user is a member of a group (direct member). That group in turn has a parent group. The user is an indrect member of that parent group.
:::
...
Use the following example to list groups that a user object is a direct member of, but filtered based on group name:
