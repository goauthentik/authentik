...
- `password_device`: User's local password authenticator, if one is configured. Its `password_change_date` property contains the date that the password was last changed. Read-only.
