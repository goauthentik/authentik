...
You can create advanced queries to locate specific groups within the list shown under **Directory** > **Groups** in the Admin interface. Use the auto-complete in the **Search** field or enter your own queries to return results with greater specificity.
...
- **Field**: `name`, `is_superuser`, `attributes`
...
- **Example queries**:
    - search group by name: `name = "admins"`
    - search group by superuser status: `is_superuser = True`
    - search group by attribute: `attributes.my_custom_attribute = "foo"`
