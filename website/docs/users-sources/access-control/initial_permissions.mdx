...
:::info
Typically, initial permissions are assigned to a user or role which is not a super-users or Admin; in this case the Admin, when implementing the nitial permissions, will need to assign the `Can view Admin interface permission` in order for the user(s) to access the Admin interface. For details see Step 5 below.
:::
