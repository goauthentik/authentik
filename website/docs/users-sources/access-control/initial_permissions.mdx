...
An authentik Admin creates an initial permissions object (a set of selected permissions) and then associates with with either an individual user or with a role (in which case everyone in a group with that role will have the same initial permissions).
