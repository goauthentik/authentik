...
An **internal** user is typically a team member, such as a company employee, who has access to the full Enterprise feature set inlcuding the application dashboard and self-service functionality.

An **external** user might be an external consultant, a volunteer in a charitable site, or a B2C customer. External users can't access the application dashboard, nor self-serivce funcionality. Instead, external users are typically authenticated and then redirected to log directly into their [default application](../sys-mgmt/brands.md#external-user-settings).
