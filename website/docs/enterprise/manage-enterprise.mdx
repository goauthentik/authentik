...
License validation and user counting happen locally. The installation does not need access to the Customer Portal or a licensing server. See [air-gapped licensing](./get-started.mdx#air-gapped-environments) for transferring keys into a disconnected installation.
...
Verify the new expiry date and capacity on the **Licenses** page. Renewing the subscription in the Customer Portal does not update the key stored in your installation; install the replacement key before the old one expires.
...
| The combined license expired over six weeks ago, or capacity is exceeded and the last valid usage record is over six weeks old | The installation enters read-only mode.                                                            |

Capacity thresholds use the last recorded valid status, not the date a key was installed. If the installation has no valid usage record and its users exceed the licensed capacity, it can enter read-only mode immediately. Purchase enough capacity for both user types before installing the first key.
...
Before purchasing a license, copy the authentik installation's [Install ID](./get-started.mdx#1-copy-the-install-id).
