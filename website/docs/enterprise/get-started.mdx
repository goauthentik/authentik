...
If authentik rejects the key, verify that you copied the complete key and that its Install ID matches the current installation. Contact [Enterprise support](./enterprise-support.mdx) if a valid key still cannot be installed.

## Next steps

- Review the [Enterprise features](./enterprise-features.mdx) and configure the features that you need in the standard authentik documentation.
