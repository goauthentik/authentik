...
- Review the [Backup and restore](../../sys-mgmt/ops/backup-restore.mdx) guidance before proceeding.
...
- refresh PostgreSQL's planner statistics if authentik requests are slow after the upgrade. See [Troubleshoot slow PostgreSQL queries](./slow-queries.mdx).
