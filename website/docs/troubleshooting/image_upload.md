...
:::info
This section applies to Docker Compose and other bind-mount based deployments where the host filesystem permissions control whether authentik can write to the mounted data path.
:::
...
For Docker Compose, run these commands in the folder of your Compose file:
...
If you are using Kubernetes or another platform, ensure the volume mounted at `/data` is writable by the authentik container instead of using the commands above directly.
...
For Kubernetes or other custom deployments, mount your persistent storage at `/data` instead of `/media`.
