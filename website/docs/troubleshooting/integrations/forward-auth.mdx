...
Temporarily raise the log level to `trace` so the outpost records every header it receives. Follow the platform-specific steps in the [Logs troubleshooting guide](../getting-started/logs.mdx#enabling-trace-mode) to enable trace mode.
