...
Starting with authentik 2024.8, RADIUS provider property mappings can be created, which make it possible to add custom attributes to the RADIUS response packets.
...
After creation, the RADIUS property mapping needs to be selected in the RADIUS provider.
