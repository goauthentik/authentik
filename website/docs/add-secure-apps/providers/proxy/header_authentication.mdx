...
:::info
To let upstream applications cryptographically verify the user's identity instead of trusting plain identity headers, the proxy also forwards a signed JWT. See [Verifying identity with the signed JWT](./index.md#verifying-identity-with-the-signed-jwt).
:::
