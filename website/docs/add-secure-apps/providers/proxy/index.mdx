...
If an upstream application cannot handle non-ASCII header values, use a [scope mapping to percent-encode them](./custom_headers.mdx#percent-encode-non-ascii-header-values).
