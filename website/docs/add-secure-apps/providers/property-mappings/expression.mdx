...
-   `request`: The current request. This may be `None` if there is no contextual request. See ([Django documentation][def])
-   Other arbitrary arguments given by the provider, this is documented on the provider.

[def]: https://docs.djangoproject.com/en/3.0/ref/request-response/#httprequest-objects
