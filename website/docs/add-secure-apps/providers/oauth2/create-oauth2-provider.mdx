...
The client secret of a confidential client is a [Secret](../../../sys-mgmt/secrets/index.mdx) object. Leave the field empty and authentik creates one with a generated value, or select an existing secret to share it between providers. [Rotating the secret](../../../sys-mgmt/secrets/rotation.mdx) replaces the value: clients still using the old one are rejected, and if the provider has no signing key, ID tokens signed with the old value no longer validate, so update every client.
