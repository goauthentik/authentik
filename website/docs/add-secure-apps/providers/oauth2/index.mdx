...
A server-to-server notification mechanism that allows an Identity Provider to notify connected OAuth2/OpenID clients when a user's session has been terminated.

For more information, see [OAuth2/OpenID Connect Back-Channel Logout](./backchannel-logout.mdx).
