...
The token introspection and revocation endpoints are global OAuth 2.0 endpoints, but access to tokens remains scoped by provider. A confidential provider can introspect tokens that it issued. Both public and confidential providers can revoke their own tokens.

To allow confidential provider B to introspect or revoke tokens issued by provider A, open provider A and add provider B under **Federated OAuth2/OpenID Providers**. Authenticate introspection and revocation requests using provider B's client credentials. This permission is directional and does not grant provider A access to provider B's tokens.
...
- `bound_key`: Requests an ID token bound to a client-held key using OpenID Connect Key Binding and Demonstrating Proof of Possession (DPoP). See [OpenID Connect key binding](./key-binding.mdx) for configuration, proof requirements, and a client example.
