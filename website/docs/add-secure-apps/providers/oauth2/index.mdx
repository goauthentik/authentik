...
OpenID Connect clients can request account selection with `prompt=select_account`. When this prompt is present, authentik starts the brand's configured [user selection flow](../../flows-stages/stages/user_selection/index.md) before continuing authorization. authentik also starts that flow automatically when the same browser has multiple live accounts available for the provider authorization request.

Clients can send `login_hint` with `prompt=select_account` to place the matching browser-local account first. The hint does not select or authenticate the account by itself.
