...
Due to how the OAuth2 provider endpoints are structured, you cannot create applications with OAuth2 providers that use the slugs `authorize`, `token`, `userinfo`, or `revoke` as these would conflict with the global OAuth2 endpoints.
:::
