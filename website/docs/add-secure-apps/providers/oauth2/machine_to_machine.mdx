...
To include custom claims from scope mappings in the issued JWT, select the scope mappings on the OAuth2 provider and request their scope names in the token request with the `scope` parameter, for example `scope=profile custom-scope`.
...
To use the OAuth introspection endpoint to obtain more information on a token, authenticate with a confidential provider. By default, a confidential provider can introspect tokens that it issued. Both public and confidential providers can revoke their own tokens.

For cross-provider introspection and revocation configuration, refer to [Cross-provider token introspection and revocation](./index.mdx#cross-provider-token-introspection-and-revocation).
