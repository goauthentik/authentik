...
To use the OAuth introspection endpoint to obtain more information on a token, authenticate with a confidential provider. By default, a confidential provider can introspect tokens that it issued, and a provider can revoke tokens that it issued.
