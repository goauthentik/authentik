...
authentik treats a grant type of `password` the same as `client_credentials` to support applications which rely on a password grant. Scopes must be defined and have to match what's available on the corresponding provider.
...
For both externally and authentik issued JWTs, authenti will create a service account which is based on the provider name and the subject claim of the token that authentik is given. You many end up with multiple service accounts depending on the tokens that are provided to authentik. These service accounts will expire and be deleted based on the expiry claim of the token that authentik is given.
...
If you're authorizing with a JWT, then `request.context["oauth_JWT"]` is available which is the parsed JWT.
...
If you receiver any error response from authentik it will only be a generic error, error description, and the `request_id`.

However, more detailed error information can be obtained from the authentik server container logs by searching for the `request_id` that you're experiencing issues with.

### OAuth introspection endpoint

To use the OAuth introspection endpoint to obtain more information on a token, you must first to authenticate to it.

You are only able to introspect a token from the same provider that was used to authenticate, or you must configure any other provider which tokens need to be introspected as a federated OIDC provider on the authenticating provider.
...
| [Static authentication and automatically generating the service account](#automatic-service-account-creation) | `oauth_client_secret` | `none`                                       |
