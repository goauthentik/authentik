...
You can [export WS-Federation metadata](./create-wsfed-provider.mdx#export-authentik-ws-federation-provider-metadata) from an authentik WS-Federation provider to an STS to automatically provide important endpoint and certificate information to the SP.
...
For details, refer to our [SAML certificates documentation](../saml/index.mdx#certificates).
...
The same property mappings that are used in WS-Federation are used in SAML. For details, refer to our [SAML property mapping documentation](../saml/index.mdx#certificates).

## Attributes for WS-Federation

WS-Federation and SAML also share the use of the [NameID](../saml/index.mdx#nameid) and the [AuthnContextClassRef](../saml/index.mdx#authncontextclassref) attributes.
