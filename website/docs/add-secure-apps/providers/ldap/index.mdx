...
A user must have access to the LDAP application before they can bind and search the directory. If the application uses [bindings](../../applications/manage_apps.mdx#use-bindings-to-control-access), the user must satisfy them.
...
- **Object permission** _(recommended)_: Open the LDAP provider's **Permissions** tab, click **Assign Object Permissions**, select the role, enable **Search full LDAP directory**, and click **Assign**. This permission applies only to that LDAP provider.
- **Global permission**: Open the role's **Permissions** tab and assign **Search full LDAP directory** under **Assigned global permissions**. This permission applies to every LDAP provider. For detailed instructions, see [Manage permissions](../../../users-sources/access-control/manage_permissions.mdx#assign-or-remove-permissions-for-a-specific-role).
