...
3. Select **Password stage** as the stage type, click **Next**, and set the following required configurations:
    ...
    - For **Backends**, include **User database + app passwords** to allow [binding with app passwords](./index.mdx#bind-with-an-app-password). Keep **User database + standard password** selected if users should also be able to bind with their regular passwords.
4. Click **Finish**
...
The service account must have access to the LDAP application. If the application uses [bindings](../../applications/manage_apps.mdx#use-bindings-to-control-access), configure them to allow the service account access.

To search for other users and groups, the service account also needs the **Search full LDAP directory** permission. Create a role with this permission and add the service account to it:

1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Directory** > **Roles** and click **New Role**.
3. Provide a name for the role (e.g. `LDAP search`) and then click **Create Role**.
...
8. Click **Assign Role Object Permission**.
9. Select the role that you created (e.g. `LDAP search`), enable the **Search full LDAP directory** permission, and then click **Assign Role Object Permission**.

This object permission grants the service account full search access to this LDAP provider only. For more information, see [Searching](./index.mdx#searching).
