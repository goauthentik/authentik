...
9. Select the role that you created (e.g. `LDAP search`), enable the **Search full LDAP directory** permission, and then click **Assign**. This creates an object permission that grants the service account access to this provider's directory without granting it access to every LDAP provider. See [LDAP Provider Searching](./index.mdx#searching) for more details.
