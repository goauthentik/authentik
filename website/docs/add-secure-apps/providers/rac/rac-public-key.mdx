...
Connection settings for a single device are set with a [RAC property mapping](./index.mdx#rac-property-mappings), which receives the device it is evaluated for:

```python
if device.name == "my-server":
    return {
...
The equivalent of the provider-wide key above, for one device:
`yaml
    private-key: |
        -----BEGIN SSH PRIVATE KEY-----
        SAMPLEgIBAAJBAKj34GkxFhD90vcNLYLInFEX6Ppy1tPf9Cnzj4p4WGeKLs1Pt8Qu
