...
The RAC provider supports SSH public key authentication. This allows for secure connections to SSH devices without the use of passwords.
...
4. In the **Settings** codebox enter the private key of the device, for example:
    ```yaml
    private-key: |
...
## Apply a private key to a single device

1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Providers**.
3. Click the name of the RAC provider that the device is reachable through.
4. Under **Devices**, click on the **Edit** icon next to the device that requires public key authentication.
5. Under **Advanced settings**, in the **Settings** codebox enter the private key of the device:
    ```yaml
    private-key: |
        -----BEGIN SSH PRIVATE KEY-----
        SAMPLEgIBAAJBAKj34GkxFhD90vcNLYLInFEX6Ppy1tPf9Cnzj4p4WGeKLs1Pt8Qu
