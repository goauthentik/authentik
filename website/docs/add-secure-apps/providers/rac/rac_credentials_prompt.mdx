...
You can configure the RAC provider to prompt users for their credentials when connecting to a device. This is particularly useful for establishing RDP connections to modern Windows systems that often require credentials to establish a connection.

After implementing this configuration, when connecting to a device users are prompted to enter their credentials which are then passed to the device. This means that static credentials do not need to be set in the RAC provider, property mapping, or on the device.
...
## Update the RAC provider settings _(sometimes required)_
...
1. Log in to authentik as an administrator and open the authentik Admin interface.
2. Navigate to **Applications** > **Providers** and click the **Edit** icon of the RAC provider that you're using.
3. Under **Protocol settings** in the **Settings** box, enter `security: tls`
...
Log in to authentik with a user account that has the required privileges to access the RAC application. Open the User interface, and on the **Application Dashboard** page click the RAC application. You should then be redirected to the prompt stage and prompted for a username and password. Enter the credentials for the device and if the credentials are valid the RDP/SSH/VNC connection should be established.
