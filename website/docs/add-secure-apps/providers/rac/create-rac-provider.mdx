...
For an overview of Remote Access Control (RAC), see the [RAC provider](./index.mdx) documentation.
...
<iframe
    width="560"
    height="315"
    src="https://www.youtube.com/embed/9wahIBRV6Ts?start=22"
...
5. On the **Configure Remote Access Provider** page, provide the configuration settings:
    ...
    - **Delete authorization on disconnect**: leave disabled, the default, to allow the browser to reuse a valid authorization when reconnecting after a disconnect. Enable it to require a new authorization after every disconnect, including temporary network failures. authentik deletes the stored authorization during connection setup, so it is already unavailable for reconnecting when the connection ends. This setting does not control the remote operating system's login session. See [Connection management](./index.mdx#connection-management) for details about authorization reuse and expiry.
    - **Device access group _(optional)_**: limit this provider to the devices in a single [device access group](../../../endpoint-devices/manage-devices.mdx). When left empty, every device that a user has access to can be reached through this provider.
    - **Protocol**: select **Automatic** to connect with RDP to devices running Windows and with SSH to every other device, or pick a single protocol for all devices of this provider.
...
Refer to the [RAC Credentials Prompt](./rac_credentials_prompt.mdx) and [RAC SSH Public Key Authentication](./rac-public-key.mdx) documentation for alternative methods of handling RAC authentication.
...
4. On the **New RAC Provider Property Mapping** page, provide the following configuration settings:
    ...
    - Under **Advanced settings**:
        - **Expression _(optional)_**: define other connection settings to be used, such as an SSH key. For more information, refer to the [Connection settings](./index.mdx#connection-settings) documentation.

5. Click **Finish**.

### Add the devices to connect to

RAC connects to the [devices](../../../endpoint-devices/index.mdx) in your authentik instance. A device that is already enrolled through a connector, such as the authentik agent, can be used as-is: authentik takes its address and operating system from the facts the device reports.

For a remote machine that is not enrolled, such as a server, add it manually:
...
4. On the Provider page, under **Devices**, click **New Device**, and provide the following settings:
    ...
    - **Host**: enter the host name or IP address of the remote machine, optionally including the port. Leave empty to use the address the device reports.
    - **Protocol**: leave as **Automatic** to use the provider's protocol, or select the protocol for this device.
    - **Advanced settings _(optional)_**: set a device-specific connection limit and other connection settings. For more information, refer to the [Connection settings](./index.mdx#connection-settings) documentation.

5. Click **Create**.

:::info
Anyone who can view a device can see its attributes, so do not store credentials in a device's connection settings. Use a [RAC property mapping](#create-rac-property-mappings) or the [credentials prompt](./rac_credentials_prompt.mdx) instead.
:::
