...
RAC connects to the [devices](../../../endpoint-devices/index.mdx) in your authentik instance. A device that is already enrolled through a connector, such as the authentik agent, can be used as-is: authentik connects to it on the address it reports.
...
The same form is used to give a device that _is_ enrolled a connection override, for example when it must be reached on a jump host address rather than the one it reports.

:::info
Connection credentials belong in a [RAC property mapping](#create-rac-property-mappings) or the [credentials prompt](./rac_credentials_prompt.mdx), not in a connection override.
:::
