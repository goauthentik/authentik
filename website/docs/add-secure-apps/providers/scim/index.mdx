...
- Patch updates
    ...
    If the service provider supports [PATCH updates](https://datatracker.ietf.org/doc/html/rfc7644#section-3.5.2), authentik uses PATCH requests to update users and groups, and to add/remove members of groups. For users, authentik only replaces the attributes that have changed, and sets `externalId` only when it creates the user. Otherwise, authentik uses PUT requests.
