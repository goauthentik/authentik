...
For instructions on creating a SCIM provider, refer to the [Create a SCIM provider](./create-scim-provider.mdx) documentation.

## Authentication modes

In authentik, there are three ways to authenticate SCIM requests:

- **Static token** provided by the application. This is the default authentication mode.
- **Basic** authentication, using a username and password provided by the application.
...
When the authentication mode is set to **Basic**, authentik sends the username and password provided by the application as an [HTTP Basic](https://www.rfc-editor.org/rfc/rfc7617) `Authorization` header with each outgoing SCIM request. Use this mode for applications that do not accept a bearer token.
...
### Inspect advertised resource types

To inspect the resource types advertised by a destination, open the SCIM provider's **Overview** tab in the Admin interface and click **Fetch resource types** in the **Advertised resource types** card.

The card displays the name, endpoint, and schema of each resource type returned by `/ResourceTypes`. Expand **Details** to inspect its description, ID, and schema extensions. The diagnostic distinguishes a successful empty listing from an unavailable endpoint or a failed request.

Discovery runs only when requested. Results use the **Service Provider Config cache timeout** setting and are cached separately from `ServiceProviderConfig`. Updating the provider clears the cache. Click **Refresh** to bypass the cache and query the destination again. Failed requests are not cached.

This diagnostic does not change synchronization behavior. An advertised list containing only users does not yet prevent authentik from sending group requests.
