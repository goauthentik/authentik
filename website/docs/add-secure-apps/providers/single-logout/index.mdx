...
- **Logout from authentik**: When a user logs out directly from authentik, the `default-invalidation-flow` runs. This flow includes a [User Logout stage](../../flows-stages/stages/user_logout/index.mdx) that ends the user's authentik session and triggers Single Logout for all connected applications.
...
2. Add stages as needed:
    - Include a [User Logout stage](../../flows-stages/stages/user_logout/index.mdx) if you want full Single Logout.
...
See the [SAML Single Logout documentation](../saml/saml_single_logout.mdx) for detailed instructions. You will need to:
...
- [SAML Single Logout](../saml/saml_single_logout.mdx)
