...
The shared secret is a [Secret](../../../sys-mgmt/secrets/index.mdx) object. Leave the field empty on create and authentik creates one with a generated value. When the secret is [rotated](../../../sys-mgmt/secrets/rotation.mdx), the outpost picks up the new value within a few seconds, and RADIUS clients still using the old one are rejected until you update them.
