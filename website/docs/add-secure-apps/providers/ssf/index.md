...
An SSF provider allows applications to subscribe to _security signals_, translating them into Security Event Tokens (<abbr>SET</abbr>) that are captured by the IdP. In this arrangement, authentik acts as an mediator from an external event _transmitter_, ultimately with a specific application acting as the _receiver_ of the events.
...
The identity provider is the transmitter of _identity signals_ through authentik to the application,
and the application is the receiver of _identity signals_, such as user information, groups, and roles.

An SSF provider is the transmitter of _security signals_ through authentik to the application,
...
:::info Roles vary by integration

The arrangement of transmitter and receiver will vary depending on the integration. And in some cases, such as IdP-to-IdP communication, the transmitter and receiver can be different systems entirely.
...
Another example use case is when an application uses SSF to subscribe to authorization events because the application needs to know if a user changed their password in authentik. If a user changes their password in authentik, an event will be generated and sent to the application, which can then take action based on the content of the event.

## SSF in authentik

After an SSF provider is configured in authentik, it needs associating with an application as [backchannel provider](../../applications/manage_apps#backchannel-providers). Backchannel providers are used to augment the functionality of the main provider for an application. Thus, you will still need to [create a typical application/provider pair](../../applications/manage_apps#instructions) (using an OIDC provider), and when creating the application, assign the SSF provider as a backchannel provider.
...
When creating the SSF provider, you will need to select a signing key to sign Security Event Tokens (<abbr>SET</abbr>). This key is used to sign the SETs that are sent to the application. The application will use this key to verify the SETs.

You may also specify an event retention period. If an event could not be sent correctly, and retries occur, the event's expiration is also increased by this duration.
