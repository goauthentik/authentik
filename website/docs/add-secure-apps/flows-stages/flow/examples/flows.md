...
There's also [a version](/blueprints/example/flows-recovery-email-verification.yaml) of this flow available without MFA validation at `example/flows-recovery-email-verification.yaml`, which is not recommended.
