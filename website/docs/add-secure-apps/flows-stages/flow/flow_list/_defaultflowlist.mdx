...
- **Invalidation**: designates a default flow to be used to invalidate a session. There are two default invalidation flows:
    - `default-invalidation-flow`: Used when users log out directly from authentik. This flow **includes** a [**User Logout**](../../stages/user_logout.md) stage, which ends the authentik session and triggers [Single Logout](../../../providers/single-logout/index.md) for all connected applications.
