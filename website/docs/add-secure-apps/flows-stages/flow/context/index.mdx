...
The invitation used with the invitation stage. When _Continue flow without invitation_ is enabled, this may be unset.
...
A boolean value which is `True` when an invitation has been used.
