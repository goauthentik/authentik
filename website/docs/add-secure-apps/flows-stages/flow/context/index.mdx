---
title: Flow Context
description: An overview of the data stored in a flow context and how authentik uses it.
...
Policies can change this data. Stages such as [User Write](../../stages/user_write/index.mdx) can then store values from `prompt_data` on `pending_user`.

For more information, see [Collect custom user attributes during enrollment](../collect-user-attributes.mdx).

#### `policy_results` (dictionary):ak-version[2026.11]

Contains the latest result from each policy binding evaluated during the flow. Results are keyed by policy binding UUID. Each result contains the policy name, whether the policy passed, its messages, and its raw return value.

```json
{
    "policy_results": {
        "3f86c2ce-48f9-4d4a-b8ef-4985b64c9c25": {
            "policy": "risk-score",
            "passing": true,
            "messages": [],
            "raw_result": {
...
Policy bindings in the same policy-engine execution are evaluated concurrently. Their results become available only after that execution finishes, so a policy can consume results from an earlier flow or stage evaluation but not from a sibling binding in the same evaluation.

The `raw_result` value is stored as a detached snapshot. Recursive references are replaced with `null`, non-JSON dictionary keys are converted to strings, and values that cannot be copied are represented as strings.

When a stage binding has **Re-evaluate policies** enabled, the new result replaces the previous result for the same policy binding UUID.
...
The title of the consent prompt shown. Set automatically when the consent stage is used with an OAuth2, Proxy or SAML provider.
...
Boolean set to true after the email from the email stage has been sent.
