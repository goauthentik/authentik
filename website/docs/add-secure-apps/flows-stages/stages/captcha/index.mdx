...
If you embed it in an [Identification stage](../identification/index.mdx), configure the CAPTCHA provider for invisible or background use.

:::note CAPTCHA and passkey authentication
The CAPTCHA stage does not run when the user completes an Identification stage with a passkey, because authentik completes identification immediately after a successful WebAuthn check. For more information about this behavior and its security rationale, see [CAPTCHA and passkey authentication](../identification/index.mdx#captcha-and-passkey-authentication).
:::
