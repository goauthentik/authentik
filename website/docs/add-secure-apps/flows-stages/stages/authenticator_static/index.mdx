...
- Codes are displayed in hyphenated groups of four characters (for example, `abcd-efgh-ijkl`) to make them easier to read and type. Hyphens and spaces are ignored when a code is entered, so users can enter a code with or without them. Codes are case-sensitive.
