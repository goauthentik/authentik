...
Use this stage after one or more stages that populate flow context, usually an [Identification stage](../identification/index.mdx), [Prompt stage](../prompt/index.mdx), or [Email stage](../email/index.mdx).
...
An [Invitation](../invitation/index.mdx) stage adds the invitation's built-in properties and custom attributes to `prompt_data`. The User Write stage stores an `attributes` dictionary or dotted keys such as `attributes.department` in the same way as submitted prompt values. For more information, see [Invitations](../../../../users-sources/user/invitations.mdx).
