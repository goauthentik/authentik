...
To slow down brute-force attacks against code-based authenticators, the stage applies an exponential back-off to each device after a failed verification attempt. The delay required between verification attempts grows with each successive failure, as:
...
WebAuthn and Duo devices are not throttled by this stage.
