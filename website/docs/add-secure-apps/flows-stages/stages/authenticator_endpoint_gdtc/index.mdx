...
- **Credentials**: a secret containing the Google service-account JSON used to access the Chrome Verified Access API.
...
6. In the stage's **Credentials** field, create or select a secret containing the exported JSON key. Use the **File** type to upload it, or **Multi-line text** to paste its contents.
