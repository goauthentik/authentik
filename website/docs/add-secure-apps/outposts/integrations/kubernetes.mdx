...
After installation, the chart outputs an example kubeconfig file. Create a Kubernetes service connection in authentik and select a secret in **Kubeconfig**. Upload the kubeconfig as a **File** secret, or paste its YAML into a **Multi-line text** secret.
