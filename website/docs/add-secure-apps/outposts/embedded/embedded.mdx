...
:::warning
The embedded outpost is not currently supported with [multi-tenancy](../../../sys-mgmt/tenancy.md). If you enable tenants, disable the embedded outpost with `AUTHENTIK_OUTPOSTS__DISABLE_EMBEDDED_OUTPOST=true`.
:::
...
If this is not correct, navigate to **Applications** > **Outposts**, edit **authentik Embedded Outpost**, and set `authentik_host` to the URL that users should use to log in.
