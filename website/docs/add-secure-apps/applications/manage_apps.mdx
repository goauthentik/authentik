...
You can control access to an application by using bindings, defined in the application's configuration. The most common ways to do this are: 1. [create a policy binding](../../customize/policies/working_with_policies.md#bind-a-policy-to-an-application) in which a policy is used to determine whether or not a user can access an application. 2. bind a user or group to the application.
