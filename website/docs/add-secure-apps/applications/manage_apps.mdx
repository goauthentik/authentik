...
To hide an application without modifying its policy settings or removing it, enable the _Hide from user dashboard_ toggle on the application. The application will no longer appear on the **My applications** page.
