...
- **Choose a Provider type**. The protocol for a backchannel provider must be SCIM, LDAP, GWS, Entra, or SSF.
- **Configure the Provider**: - Do NOT add any value in the **Provider** field (doing so will cause the provider to display as an application on the user interface). - Enter any required configurations.

3. Click **Finish** to save the provider.
4. Edit the application by going to **Applications** > **Applications** and clicking the edit icon.
