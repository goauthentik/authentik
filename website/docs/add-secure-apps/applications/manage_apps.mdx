...
By default, if you click **New Application**, you are prompted to create the new application and a new provider. However, you can use the drop-down menu beside **New Application** to explicitly select either:
    ...
    - Select **with New Provider** to create the application and configure a new provider at the same time.
    - Select **with Existing Provider** if you have already created a provider separately, and now you want to create the application and connect it to the provider.
:::
