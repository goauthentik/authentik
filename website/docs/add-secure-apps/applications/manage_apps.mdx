...
:::info
In earlier versions, hiding an application was done by setting the **Launch URL** to `blank://blank`. Existing applications using that value are migrated automatically to the **Hide from user dashboard** toggle on upgrade.
:::
