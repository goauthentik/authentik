...
description: "Define reusable object attributes for authentik objects"
...
Before authentik 2026.8, an object's attributes were edited as free-form YAML or JSON in an object's **Attributes** field. These attributes had no discoverable keys, human-readable labels, or validation. Object attributes improve that experience without changing how attributes are stored: they remain in an object's **Attributes** field.
...
authentik includes definitions for common object attributes. These default object attribute definitions are currently disabled by default.
...
### Examples of using object attributes in property mappings

SAML property mapping:

```python
return user.attributes.get("given_name", user.name)
```

Nested attribute value, for example a locale claim in an OAuth2 scope mapping:

```python
return {
    "locale": user.attributes.get("settings", {}).get("locale", "en-US"),
}
```

Always use `.get()` with a fallback. An object attribute definition does not guarantee that every object has a value for that key.

## Validation

### Uniqueness

If an object attribute definition is set as `Attribute is unique`, authentik rejects a save when another object already stores the same value for that key. Uniqueness is only enforced while the object attribute definition is enabled.

### Existing data

Creating an object attribute definition does not validate values that are already stored on existing objects. Validation is applied when an object is saved. An object that was created before the object attribute definition existed can therefore hold a value that does not match the object attribute definition's regex.

## Considerations and limitations

- Array values are not currently supported. Only Text, Number, or Boolean values are supported. Support for array values is tracked in [this issue](https://github.com/goauthentik/authentik/issues/24250).
