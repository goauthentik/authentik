...
Whenever any of the following actions occur, an event is created. Actions are used to define [Notification Rules](notifications.mdx).
...
A secret value, such as a token or managed Secret, was replaced. See [Rotate a secret](../secrets/rotation.mdx).
...
You can use these events in notification rule expression policies to automate actions, such as [creating LDAP users from authentik events](./notification_rule_expression_policies.mdx#create-ldap-users-from-authentik-events).
