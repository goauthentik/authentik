...
For the built-in ways to send a set-password link, see [User credentials recovery](../../users-sources/user/user_basic_operations.mdx#user-credentials-recovery).
:::

## Create LDAP users from authentik events

Use this example when you create users in authentik and also need entries for them in an external LDAP directory, such as one used by other applications. LDAP source synchronization only imports users into authentik, so creating entries in the external directory requires custom automation.

This expression policy creates an `inetOrgPerson` entry on an external LDAP server for each user `model_created` event. It leaves existing LDAP entries unchanged and does not synchronize later changes or link the accounts.

Replace `my-ldap-source` with an enabled LDAP source's slug and `ou=users,dc=example,dc=com` with an existing target DN. The source's bind account needs permission to add entries there. Adapt the object classes and attributes, including the surname `sn`, to your directory.
...
# Only handle user creation events, not updates or events for other models.
event = request.context.get("event")
if not event or event.action != "model_created":
    return False
...
# The user may have been deleted before this event is processed.
user = User.objects.filter(pk=model.get("pk")).first()
if not user:
    return False

# Replace the slug to use your source's connection settings and bind credentials.
source = LDAPSource.objects.get(slug="my-ldap-source", enabled=True)
conn = source.connection()
try:
    conn.add(
        # Replace the parent DN with your target container. Escape the username
        # so special characters cannot change the structure of the entry's DN.
        f"uid={escape_rdn(user.username)},ou=users,dc=example,dc=com",
        # Adapt these classes and attributes to your directory's schema.
        object_class=["top", "person", "organizationalPerson", "inetOrgPerson"],
        attributes={
            "uid": user.username,
            "cn": user.name or user.username,
            # inetOrgPerson requires sn. Use your surname attribute if available.
            "sn": user.name or user.username,
        },
    )
except LDAPEntryAlreadyExistsResult:
    # Reprocessing an event must not overwrite an existing LDAP entry.
    ak_logger.info("LDAP entry already exists; leaving it unchanged")
finally:
    conn.unbind()

# Suppress the notification object after performing the LDAP write.
return False
```

[Bind the expression policy to a notification rule](./notifications.mdx#3-create-a-notification-rule-and-bind-it-to-the-policy). No notification destination or transport is needed; returning `False` suppresses the notification after the LDAP write. LDAP source sync does not emit `model_created` events for imported users.
