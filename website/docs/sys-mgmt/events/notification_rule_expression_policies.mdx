...
authentik must be able to send email before you can use `ak_send_email`. To do this, either configure the [global email settings](../../install-config/email.mdx), or set up an [Email stage](../../add-secure-apps/flows-stages/stages/email/index.md) and pass it to `ak_send_email` via its `stage` argument to use that stage’s connection settings instead.
