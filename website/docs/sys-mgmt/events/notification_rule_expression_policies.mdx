...
authentik needs to be able to send mail. Either configure a [global email connection](../../install-config/install/docker-compose.mdx#email-configuration-optional-but-recommended), or pass an [Email stage](../../add-secure-apps/flows-stages/stages/email/index.md) to `ak_send_email` through its `stage` argument to use that stage's connection settings instead.
...
Matching on `model_created` ensures the email is sent only when the account is first created. Other changes to the user, such as a password reset, are recorded as `model_updated` and will not trigger this policy.
