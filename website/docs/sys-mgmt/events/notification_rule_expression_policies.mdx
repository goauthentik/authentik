...
:::info
authentik ships with a built-in welcome email as part of the default [user security notifications](./user-security-notifications.md): enable the `default-notify-user-welcome` notification rule to use it. Use the pattern below when you need more control than the built-in rule offers — for example, different content per user type, additional conditions, or a fully custom message.
:::

This expression policy sends a welcome email to a user when their account is created through an enrollment flow, the Admin interface, the API, or a blueprint. Instead of creating an administrator notification, it uses [`ak_send_email`](../../customize/policies/types/expression/reference.mdx) to email the new user directly. The policy matches the [`user_created`](./event-actions.md#user_created) event:ak-version[2026.8] and uses its context fields to skip service accounts and accounts that are bulk-imported by a source sync.
...
# Only email regular accounts, not service accounts.
if event.context.get("user_type") not in ("internal", "external"):
    ak_logger.info("user type does not match")
    return False

# Only email accounts that were deliberately created for someone — via the
# Admin interface, the API, an enrollment flow, or a blueprint. Skip accounts
# bulk-imported by a source sync.
if event.context.get("origin") not in ("http", "blueprint"):
    ak_logger.info("origin does not match")
    return False

user = User.objects.filter(uuid=event.context.get("subject_uuid")).first()
...
4. Leave **Send notification to event user** and **Send notification to affected user** disabled, leave **Destination group** empty, and do not select a notification transport.
...
The `user_created` event is only created when the account is first created, so later changes to the user do not re-trigger the policy. The event's `origin` context field records how the account was created, which the policy above uses to skip accounts imported by a source sync.
