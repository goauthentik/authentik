...
To receive notifications about events, first [create a notification transport](#create-a-notification-transport), and then define a notification rule with a bound policy. For more information, see the [workflow overview](./notifications.mdx#workflow-overview).
