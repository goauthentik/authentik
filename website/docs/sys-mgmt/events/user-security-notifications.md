...
| `default-notify-user-mfa-device-change`   | An MFA device is added to or removed from their account ([`mfa_device_created` / `mfa_device_deleted`](./event-actions.md#mfa_device_created--mfa_device_deleted)).                                                                                                                                    |
...
| `default-notify-user-account-offboarding` | Their account expires through a scheduled [user offboarding](../user-offboarding.md) with the **Deactivate** action ([`user_offboarded`](./event-actions.md#user_offboarded)), an Enterprise feature. Offboardings that delete the account, and deactivating a user directly, do not trigger an email. |
