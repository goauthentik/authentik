...
A third theme — `reduced` — is reserved for users who explicitly opt into reduced motion, in addition to the browser-level `prefers-reduced-motion` media query, which fires automatically. authentik respects both, so most operators typically do not need to do anything to support reduced motion users.
