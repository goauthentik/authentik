...
Most brand customization stays at the first layer. The lower layers are escape hatches.

## Token groups

The semantic surface consists of around 60 tokens, organised by concern. Every token name follows the pattern `--ak-{group}-{member}`.
