...
For OAuth and SAML providers, in the vast majority of cases, certificate expiry does not matter. Most service providers don't check whether certificate are expired. What usually matters is that the signature is correct.

However, there are some notable exceptions to this. For example, the Slack SAML integration checks for certificate expiry.

We recommend checking your service provider's documentation for further details.

### Proxy provider and brands

We recommend using a certificate generated outside of authentik that correlates with your FQDN, preferably, generated by a publicly trusted certificate authority.
...
When a new keypair is added that has a private key which already exists in the database, authentik will automatically update the keypair's certificate in the database rather than creating a new keypair.
