...
Configure your monitoring software to send requests to `/-/health/live/`, which will return a `HTTP 200` response as long as authentik is running. Requests to `/-/health/live/` short-circuit early in the middleware pipeline without querying the database, emitting Django access logs, or recording Django request metrics.
