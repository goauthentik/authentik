...
This means that in this scenario, the maximum number of concurrent tasks will be 20. Thus, for the synchronization to run as fast as possible, there needs to be 20 available workers when it starts. However, other tasks might also be running at the same time, or might get created while the synchronization is running. Thus, we recommend having more workers than necessary to keep a buffer for those tasks.
