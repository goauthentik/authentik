...
This guide covers the settings, permissions, and migration steps that are common to S3 storage. For service-specific endpoints and authentication details, see [S3-compatible storage providers](./providers.mdx).
...
The [provider reference](./providers.mdx) lists service-specific endpoint and region values.
