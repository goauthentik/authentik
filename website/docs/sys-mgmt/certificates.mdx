...
ML-DSA (FIPS 204) is a post-quantum signature algorithm, available in the ML-DSA-44, ML-DSA-65, and ML-DSA-87 parameter sets. authentik can generate ML-DSA certificates and use them for TLS, but browsers and public certificate authorities do not yet support them. Their use is therefore limited to connections between components you control, such as outposts and mutual TLS with a private certificate authority.

ML-DSA certificates are significantly larger than RSA or elliptic curve certificates and have the following requirements:

- **Outposts**: Must run authentik 2026.11 or newer to verify ML-DSA certificates.
- **RADIUS**: Clients must use EAP-TLS 1.3, as ML-DSA requires TLS 1.3.
- **FIPS mode**: Key generation is unavailable when FIPS compliance mode is enabled because the validated OpenSSL FIPS provider does not yet implement ML-DSA.
