...
ML-DSA (FIPS 204) is a post-quantum signature algorithm, available in the ML-DSA-44, ML-DSA-65, and ML-DSA-87 parameter sets. authentik can generate ML-DSA certificates and serve TLS with them, but browsers and public certificate authorities do not support ML-DSA certificates yet, so they are currently only useful for connections between components you control, such as outposts and mutual TLS with a private certificate authority. ML-DSA certificates are also much larger than RSA or elliptic curve certificates, and outposts must run the same authentik version or newer, as older outposts cannot verify them. ML-DSA requires TLS 1.3, so RADIUS clients must use EAP-TLS 1.3 to authenticate against an ML-DSA certificate. When FIPS mode is enabled (`compliance.fips.enabled`), ML-DSA keys cannot be generated, because the validated OpenSSL FIPS provider does not yet implement ML-DSA.
