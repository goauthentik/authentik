...
Now that the backend and frontend have been setup and built, you can start authentik. The server can be started by running the following command in the same directory as your local authentik git repository:

```shell
make run-server   # Starts authentik server
```

The worker can be started by running the following command in the same directory as your local authentik git repository:
