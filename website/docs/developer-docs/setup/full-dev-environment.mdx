...
authentik uses [Redis](https://redis.io/), [PostgreSQL](https://www.postgresql.org/), [S3](https://www.zenko.io/cloudserver/), and [Sentry Spotlight](https://spotlightjs.com/). You can run these services using the docker-compose.yml file in the scripts/ directory with the following command:
...
make gen       # Generates schema files (schema.yml, blueprints/schema.json) and the TypeScript client.
```
