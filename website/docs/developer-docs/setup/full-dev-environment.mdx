...
```shell
pip install uv && \
sudo apt-get install -y libgss-dev krb5-config libkrb5-dev \
...
:::info
All `make` commands need to be run in the root of your local authentik git repository.
:::
...
Or you can live-edit the UI, allowing you to see updates you make in real time without needing to rebuild:
