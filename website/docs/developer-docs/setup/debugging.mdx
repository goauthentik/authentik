...
This page describes how to debug different components of an authentik instance, running either in production or in a development setup. To learn more about the structure of authentik, refer to our [architecture documentation](../../core/architecture.mdx).
...
The debug port needs to be accessible on the local machine. By default, this is port 9901 for the server; the worker uses `9902` and up (one port per worker process). Additionally, the container being debugged must run the debug image, `ghcr.io/goauthentik/server-debug`. It is the same build as the server image with the development dependency group added, and debugpy is part of that group. The standard server image does not ship debugpy, and it logs a warning instead of starting the debug server.

When running in Docker Compose, a file `compose.override.yml` can be created next to the authentik `compose.yml` file to select the debug image, expose the port, and enable debug mode.
...
services:
    ...
    server:
        # Use the same version as the server image in compose.yml
        image: ghcr.io/goauthentik/server-debug:<authentik-version>
        healthcheck:
            disable: true
        environment:
