...
2. **Component styles and PatternFly compatibility CSS exist at the same cascade level.** When the PatterFly base, the authentik base, and per-component styles all live at the same cascade level, ordering depends on both stylesheet adoption order and import order. Layers let us name that ordering explicitly.
...
authentik has two independent cascade contexts that do not interact:
...
These cascades are independent. Layers defined in one context do not affect the other. We can adopt layers in one context without disturbing the other.
...
This is only viable once all other styles are layered, because of the unlayered-beats-layered rule. So this is the _last_ step in the per-shadow-root context, not the first.

### Step 2 — Layer the per-shadow-root cascade

In `Base.ts`, every Lit element adopts four kinds of stylesheets in order: `PFBase`, component styles, `AKBase`, brand custom CSS. The shadow root is a self-contained cascade context, therefore we can move all four into layers in a single PR without modifying the document cascade:
...
After this step, brand custom CSS takes precedence within shadow roots without specificity duels. The document cascade is unchanged.
...
Today brand custom CSS is per-shadow-root. Authors who want to target light-DOM elements (server-rendered flow pages, headers) cannot reach them. A future change could also inject branding CSS into the document head wrapped in `@layer overrides`. This is out of scope for this RFC.

## Risks and how to detect them

- **Specificity inversion.** Moving a `.foo` rule into a layer demotes it relative to any unlayered `.foo`. Detection: visual regression against Storybook and flow executor stories before and after each step.
- **PatternFly internals depending on cascade order.** PatternFly's own component CSS sometimes relies on order-of-import. Detection: confine layering to authentik-authored CSS first; PatterFly stays unlayered until step 3.
