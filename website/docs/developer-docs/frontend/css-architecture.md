...
This gives good incremental control, but no stable external theming contract. Users must inspect `--pf-*` and internal `--ak-c-*` names, with no guarantee that those names remain stable across upgrades.
...
- `utilities`: narrow single-purpose utilities.
...
- Public semantic tokens should remain at approximately 30 to 60 names until real user needs justify more.
