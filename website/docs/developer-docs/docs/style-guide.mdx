...
This style guide helps keep authentik documentation consistent, clear, and easy to follow. It standardizes phrasing, formatting, tone, and structure.

We appreciate all contributions to our documentation — whether it's fixing a typo, adding new content, or writing an entirely new topic. To help us review and merge your contributions more efficiently, please follow our [writing documentation](./writing-documentation.mdx) guidelines. If you notice any inconsistencies, feel free to open an [Issue](https://github.com/goauthentik/authentik/issues) or submit a [Pull Request](https://github.com/goauthentik/authentik/pulls) to fix them.
...
Use relative paths when linking to other documentation files in this repository. Relative paths make links portable across documentation deployments, but they do not update automatically when files move. Use an absolute URL for resources outside this repository.

### File type

The standard file type for documentation is `.mdx`.

### OS-agnostic, clarify where needed

Try to write procedural (How To) docs generically enough that it does not endorse or force a specific operating system. If it is necessary to specify a specific OS be sure to label it clearly. Consider using tabs to show the different OSes.
...
Avoid phrasing that blames the user. Be objective and polite when providing instructions.
...
- Use **bold** to highlight:
    - UI elements such as field names, labels, buttons, or options (e.g., **Save** button, **Username** field).
        - Reproduce the label exactly as it appears on screen, including its capitalization. Write **Bind Flow** if the form says "Bind Flow", even though sentence case applies to headings. The reader is scanning the interface for that string, so a doc that silently re-cases it sends them looking for something that isn't there.
...
Use `note`, `tip`, `info`, `warning`, and `danger` admonitions for supplemental information. Choose the least severe type that accurately communicates the content:

- `note`: Additional context.
- `tip`: Optional advice that makes a task easier.
- `info`: Important supporting information.
- `warning`: A risk, limitation, or action that might cause problems.
- `danger`: An irreversible action or serious security or data-loss risk.
...
- For URLs entered as values or defined in fields, enclose variables in angle brackets (`< >`) and use underscores between words. See [Variables](#variables).
...
- ` ```yaml ` defines the language used for syntax highlighting. Other supported languages include `jsx`, `python`, `bash`, and `text`.
