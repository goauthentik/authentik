...
All our redirects are defined in the `website/docs/static/_redirects` file. Each line includes two three columns:

1. The source URL path (i.e the old URL to match against).
2. The destination URL path (i.e. the new URL to redirect to).
3. The HTTP status code to use when redirecting
