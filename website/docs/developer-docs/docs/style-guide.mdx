...
- When writing procedural documentation (How Tos) the steps should follow the workflow in the UI, presenting the exact page(s) they need to navigate, and then the exact fields, tabs, etc that they need to select or fill out. Present the UI components in the document in the same order they appear in the UI.
...
In general, the visual "aesthetics" of technical documentation aim to be "lean and clean." This is achieved through concise content, such as shorter sentences and clear instructions, and a layout that maintains a clean, uncluttered appearance with limited color use and minimal large callouts or announcements. Additionally, the colors light blue and light yellow are exclusively reserved for Info and Warning callouts, respectively.
