...
- Use _italic_ for:
    ...
    - Emphasis, but sparingly, to avoid overuse. For example, you can use italics for important terms or concepts on first mention in a section. Do not use italics to indicate a [variable or placeholder](#variables).
...
If there is a [colon](#following-a-colon) used in a bulletd list item, follow the capitalization rules.
