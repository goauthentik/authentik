...
Let's take a look at the file path of this page:
...
- The file path drops the `website/docs` prefix.
...
The final published URL is made possible with a combination of [Docusaurus's router](https://docusaurus.io/docs/advanced/routing) and [Netlify's redirects](https://docs.netlify.com/routing/redirects/).
...
All our redirects are defined in the `website/docs/static/_redirects` file. Each line includes two parts – the source URL path and the destination URL path:
...
```text
/core/applications    /add-secure-apps/applications/
```
...
Whether the reader is viewing page for the first time or navigating between pages, this arrangement allows us to have a single source of truth for all URLs, ensuring that each page remains consistently accessible across authentik versions and our throughout three Docusaurus deployments (Topics, Integrations, and API).
...
Moving a documentation page to a new location requires updating both the `sidebar.mjs` file in the `website/docs` directory, and the `_redirects` file.

1. Take note of the page's current URL path in the browser's address bar.
2. Move moving the Markdown file to the new location.
3. Update the `sidebar.mjs` file in the `website/docs` directory.
4. Add a new redirect rule to the `_redirects` in the `website/docs/static` directory.
