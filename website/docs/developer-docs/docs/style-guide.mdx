...
- When handling URLs:
    ...
    - For URLs entered as values or defined in fields, enclose any variables inside angle brackets (`< >`) to clearly indicate that these are placeholders that require user input.
        ...
        In Markdown, use this syntax: `https://authentik.company/application/o/<slug>/.well-known/openid-configuration`

        Rendered formatting: https://authentik.company/application/o/<slug>/.well-known/openid-configuration
