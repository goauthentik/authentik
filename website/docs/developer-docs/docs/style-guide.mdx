...
It's important to use “that” as a conjunction to introduce a dependent clause, or as a "connection" between a noun and a verb ("The provider that you created in Step 3."). Inluding "that" as a conjunction helps non-native English speakers more easily parse phrases, and improves output for translation tools.
...
For more informatin, see https://www.grammarly.com/blog/which-vs-that/.

### “since” (time-based) vs “because” (causal)

When writing about a status or anythng that is causal ("this happened because of that"), use the word "because". Use the word "since" for time-based topics (this will be rare in technical writing).
