...
### Tabs with automatic OS detection

Use **Tabs** with automatic OS detection to show OS-specific instructions. The tab for the detected OS will be selected by default, but users can switch to other OS tabs if needed. For example:
...
<Tabs
    automaticallyDetectPlatform={true}
    defaultValue="macos"
    values={[
...
>
    <TabItem value="macos">macOS-specific instructions go here.</TabItem>
    <TabItem value="windows">Windows-specific instructions go here.</TabItem>
    <TabItem value="linux">Linux-specific instructions go here.</TabItem>
</Tabs>;
```

The OS detection supports: `macos`, `windows`, `linux`, `android`, and `unknown`. 

By default, only the tabs with the right labels are detected and you must use the exact labels above for the platform to be detected.
