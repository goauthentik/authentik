...
- a successful login resets a negative score back to 0, and raises any other score by 1, for that username and client IP pair
...
When you use a Reputation policy on a flow stage binding, configure the stage binding to **Evaluate when stage is run** so authentik can use the latest request context. For more information, see [Planning and stage policies](../../../add-secure-apps/flows-stages/flow/planner.mdx#planning-and-stage-policies).
...
System-wide reputation limits and expiry are documented in [Settings](../../../sys-mgmt/settings.mdx).
