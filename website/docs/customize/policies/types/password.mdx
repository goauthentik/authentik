...
This policy is most often attached to a [Prompt stage](../../../add-secure-apps/flows-stages/stages/prompt/index.mdx) through that stage's **Validation Policies**.
...
By default, authentik's Password policy aligns with [NIST password guidance](https://pages.nist.gov/800-63-4/sp800-63b.html#password). Be careful when tightening or weakening those defaults. For broader guidance, see [Hardening authentik](../../../security/security-hardening.mdx#password-policy).
:::
...
If you also want to prevent password reuse, combine this policy with [Password Uniqueness Policy](./password-uniqueness.mdx).
