...
For simple country matching, prefer a [GeoIP policy](../geoip.mdx).
:::
...
:::info Prefer GeoIP Policy
For simple ASN matching, prefer a [GeoIP policy](../geoip.mdx).
:::
...
- `context["policy_results"]`: results from policy bindings evaluated earlier in the flow, keyed by policy binding UUID
