...
Image files are used in authentik to add icons to new applications or sources, and to define the ["branded" look](../customize/branding/index.mdx#branding-settings) of the authentik interface, with your company's logo and title, a favicon, or a background image for the flows.
...
If file uploads are missing or unavailable after an upgrade, see [Errors when uploading icons](../troubleshooting/image_upload.mdx).
...
See [File picker values](./file-picker.mdx) for the full list of supported values and path rules.
:::
