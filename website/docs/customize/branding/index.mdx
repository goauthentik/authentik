...
- **Default flow background** :ak-version[2025.4]: Default background image for the flow executor. See [Flow configuration options](../../add-secure-apps/flows-stages/flow/index.mdx#flow-configuration-options) to override this image on a per-flow basis.
...
Logo, favicon, and default flow background use the shared file picker. See [File picker values](../../customize/file-picker.mdx).
...
You can explicitly select, in your instance's Brand settings, the _default flows_ to use for the current brand. You can optionally configure these default flows ([learn more about each default flow](../../add-secure-apps/flows-stages/flow/default-flows.mdx)):
...
- **User switch flow**: the authentication flow used when [switching between users](../../users-sources/user/user-switching.mdx) signed in on the same browser. If left empty, user switching is disabled. To reuse the default authentication flow for switching, select it here explicitly.
...
When using the [Mutual TLS Stage](../../add-secure-apps/flows-stages/stages/mtls/index.mdx) and accessing authentik directly, this setting specifies which certificate authorities are trusted to issue client certificates.
