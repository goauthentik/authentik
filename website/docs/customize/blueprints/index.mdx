...
You can apply an example flow by opening the **Import** modal from either the **Flows** page or the **Blueprints** page. Select **Local path** to choose one of the example blueprints bundled with authentik, or select **File upload** to import a downloaded or exported `.yaml` file from your local file system. After applying the flow, you can decide if you want to further customize the flow, or just use it as it is.
