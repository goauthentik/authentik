...
Blueprints are used to manage authentik's default flows and other system objects. authentik's `default-authentication-flow` is, under the covers, created by a blueprint.

Custom blueprints can be used to replace or modify default blueprints. You can create a new flow that's based on the `default-authentication-flow` but adds an additional stage or policy.
...
To learn how to apply, create, modify, and manage blueprints, refer to [Working with blueprints](./working_with_blueprints.mdx).

### Blueprint instances

A Blueprint _instance_ is an instantiation of a blueprint. Each instance refers to a YAML file that's either mounted into the authentik (worker) container, in an OCI registry, or the local database. (See [Blueprint storage](#blueprint-storage) for more information.)

Multiple instances can be created from a single blueprint file. In the Admin UI, select **New Blueprint**, give the new instance a unique name, choose the blueprint file, and supply your customizations in the `context` section as key:value attributes.

authentik watches for file modification/creation events in the blueprint directory. When a new blueprint file is created, authentik will automatically trigger a _discovery_. When a blueprint file is modified, authentik will trigger an _apply_.

authentik also scans the blueprint directory on an interval and compares each blueprint file to the copy in its database. Files that are new or modified will be discovered and applied. The default interval is 60 minutes. The interval can be changed in **System Tasks > Schedules** under `authentik.blueprints.v1.tasks.blueprints_discovery`.

:::info

To prevent authentik from creating a blueprint instance automatically from a blueprint file, add or edit the YAML `metadata/labels` block and disable automatic instantiation:
...
You can apply an example flow by selecting **Import** on either the **Flows** page or the **Blueprints** page. Then, on the **Import Flow Blueprint** modal, select **Local path** to choose one of the example blueprints bundled with authentik, or select **File upload** to import a downloaded or exported `.yaml` file from your local file system. After applying the flow, you can decide if you want to further customize the flow, or just use it as it is.

This YAML file is validated and applied directly after having been uploaded or selected from an available local path. The file is not monitored afterward or automatically applied on startup like blueprint instances.

:::info
Be aware that if you [modify and save](../../add-secure-apps/flows-stages/flow/#flow-configuration-options) an existing default flow, what you get is a merged version of the two: the _original flow_ (which is based on the blueprint) _plus any changes you made to the flow_. If you want to revert any modifications that you made to the default flow and get a fresh start with the original default flow, you have to _first_ delete the modified flow, and _then_ [apply the flow's blueprint](./working_with_blueprints.mdx#import-and-apply-a-blueprint) again.
:::
...
A Blueprint is a list of entries to create, update, or delete object instances in the authentik database.

Internally, authentik is an object-oriented system of relationships which are sometimes ordered. To give just a few examples:

- Flows have ordered Bindings to Stages
- Stages have ordered Bindings to Policies
- Users have Permissions

When a blueprint is applied, to ensure database consistency and prevent incomplete configurations, all entries are processed within a single **atomic database transaction**. This means:
...
All blueprints have access to these built-in context variables:
...
The order inside a Blueprint matters. To create relationships between objects, whether defined in the Blueprint or part of the existing authentik instance, authentik provides the `!KeyOf` and `!Find` operators. These operators can only resolve an entry that already exists in the database or was already applied as an earlier entry in the current Blueprint execution transaction. For example, to create a Flow, the Flow and its Stages must come first, and then the Bindings to establish the connections between the Flow and its Stages.
...
To fetch blueprints from a private registry with authentication, credentials can be embedded in the address.
