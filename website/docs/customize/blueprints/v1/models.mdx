...
In blueprints, a user's password can also be set using the `password_hash` field. The value must follow the [supported password-hash policy](../../../install-config/automated-install.mdx#authentik_bootstrap_password_hash), such as a hash generated with the `hash_password` management command.
