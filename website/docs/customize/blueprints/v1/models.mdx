...
In blueprints, a user's password can also be set using the `password_hash` field. authentik stores the value as supplied without applying the password hash import settings. Use a Django password hash that the authentik instance can verify.
...
- model: authentik_core.user
  ...
  attrs:
      name: test user
      password_hash: <django_password_hash>
```

`password` and `password_hash` are mutually exclusive; setting both on the same user causes blueprint validation to fail.

For hash generation, verification requirements, and import behavior, refer to [Password hashes](../../../core/password-hashes.mdx).
