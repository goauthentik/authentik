...
In blueprints, a user's password can also be set using the `password_hash` field. authentik stores this value without validating its format, algorithm, or work factors. Blueprint authors are responsible for supplying a secure hash that is compatible with their authentik deployment and maintaining it as requirements change.

Use `password_hash` when you need to import or bootstrap an existing hash without exposing the raw password to authentik. You can generate a hash with the `hash_password` management command. Enter the password when prompted:
...
`password_hash` updates only authentik's local password verifier and does not propagate to LDAP or Kerberos integrations.
