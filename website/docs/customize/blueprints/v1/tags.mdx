...
```yaml
providers: !FindMany [authentik_providers_proxy.proxyprovider]  # Returns `[1, 2, 3, ...]`
```

{/* prettier-ignore-end */}

Identical to `!Find`, but returns _all_ matches' primary key, not just the first match.
