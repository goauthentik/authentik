...
entries:
    - ...
      configure_flow: !Context foo # Resolves to `some-value`
      authentication_flow: !Context some.nested.0 # Resolves to `value`
```
...
:::note
The nested value dot notation (`some.nested.0`) requires authentik 2026.11.
:::

:::tip
If a key name matches a nested dot notation key, the regular key name takes precedence over the nested key. For example:
...
entries:
  - ...
    configure_flow: !Context foo.bar # Resolves to `some-value`
```
...
```yaml
required: !Condition [
    AND, # Valid modes are: EQ, NEQ, AND, NAND, OR, NOR, XOR, XNOR
...
Normally, it should be used to define complex conditions for use with an `!If` tag or for the `conditions` attribute of a blueprint entry (see [the blueprint file structure](./structure.mdx)). However, this is essentially just a boolean evaluator so it can be used everywhere a boolean representation is required.
