...
There is a new container image, `ghcr.io/goauthentik/server-debug`. It's based on the server image, plus the development dependency group, the Kerberos server tools, and a few interactive tools. Use it for `ak test-all`, for `AUTHENTIK_DEBUGGER`, and for an interactive shell in a container.
