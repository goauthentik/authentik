...
- **Share passkeys across brands**: WebAuthn RP configs allow several brands to share one WebAuthn Relying Party ID with an explicit list of allowed origins, so credentials such as passkeys work across brands via Related Origin Requests (WebAuthn Level 3). See [Share passkeys across brands](../../add-secure-apps/flows-stages/stages/authenticator_webauthn/related-origin-requests.mdx).
