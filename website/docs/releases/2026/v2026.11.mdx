...
Administrators can require users to complete one or more flows after their next successful login by setting the `goauthentik.io/user/next-actions` attribute. authentik marks the new session as restricted, prevents it from continuing to the user interface, an application authorization flow, or an API, and releases it after the user completes every action.
