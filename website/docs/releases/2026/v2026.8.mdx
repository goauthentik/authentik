...
The `hash_password` management command no longer accepts a password as a positional command-line argument, where it was visible in the process list. The command now reads the password from a hidden prompt or from standard input. Refer to [Generate a password hash](../../core/password-hashes.mdx#generate-a-password-hash) for the current commands.
...
Before upgrading, verify that your reverse proxy sends the [required proxy headers](../../install-config/reverse-proxy.mdx#required-proxy-headers) and that every address or network from which it connects directly to authentik is included in [`AUTHENTIK_LISTEN__TRUSTED_PROXY_CIDRS`](../../install-config/reverse-proxy.mdx#trusted-proxy-networks).
...
For more details, refer to the [agent accounts documentation](../../users-sources/user/account-types/agent-accounts.mdx).
...
Users can select an area to filter the event list, and each brand can use the bundled map or configure a self-hosted map under **Map tiles**. For more details, refer to the [events documentation](../../sys-mgmt/events/index.mdx).
...
For more information, refer to the [OBO documentation](../../add-secure-apps/providers/oauth2/token_exchange.mdx#exchange-a-token-on-behalf-of-a-user-obo).
...
LDAP sources can now preserve the source directory's nested group hierarchy in authentik. Enable **Sync Group Parents** to create parent relationships between synchronized groups. For configuration details, refer to the [LDAP source documentation](../../users-sources/sources/protocols/ldap/index.mdx#additional-settings).

### Expiring policy bindings

[Policy, group, and user bindings](../../customize/policies/bindings.mdx) can now have an expiration date and time. An expired binding no longer grants access or contributes a successful policy result, which makes temporary application, flow, and stage access possible without requiring an administrator to remove the binding later.

### Base URL system setting

authentik now has a **Base URL** [system setting](../../sys-mgmt/settings.mdx) that records the external URL where the instance is available. It can be configured under **System** > **Settings**, through the API, or with [`AUTHENTIK_WEB__BASE_URL`](../../install-config/configuration/configuration.mdx#authentik_web__base_url) during installation.
...
- **CAPTCHA providers**: The [Captcha stage](../../add-secure-apps/flows-stages/stages/captcha/index.mdx) now supports JSON verification requests and the self-hostable Cap CAPTCHA provider.
...
- **Password hash imports**: [Bootstrap and API password hash imports](../../core/password-hashes.mdx) now reject malformed values before they are stored.
...
This release does not introduce new configuration options, but it more strictly enforces trusted proxy configuration. Review the [breaking change](#forwarded-headers-are-now-restricted-to-trusted-proxies) before upgrading. You can follow the upgrade instructions below; for more detailed information about upgrading authentik, refer to our [Upgrade documentation](../../install-config/upgrade.mdx).
...
- root: avoid looping indefinitely on embedded outpost start (#24553)
- root: avoid looping indefinitely on embedded outpost start (take 2) (#24824)
...
- root: fix fast shutdown log message (#24565)
...
- stages/authenticator_validate: fix intermittently failing throttling test (#24568)
...
- core: explicitly check is_active in auth backends (cherry-pick #25729 to version-2026.8) (#25761)
- core: fix bulk session revocation (cherry-pick #25816 to version-2026.8) (#25817)
- lib: avoid DNS resolution in fqdn_rand (cherry-pick #25728 to version-2026.8) (#25758)
- lib/expression: fix PickleError due to resolver match when using ak_call_policy (cherry-pick #25763 to version-2026.8) (#25768)
- lifecycle: fix server prometheus metrics getting registered early with pid instead of worker ID (cherry-pick #25883 to version-2026.8) (#25897)
- packages/django-dramatiq-postgres: prevent crashing consumer thread on schedules with unknown actors (cherry-pick #25656 to version-2026.8) (#25722)
- rbac: guard for non-existent object in object permission API (cherry-pick #25601 to version-2026.8) (#25674)
- security: automated internal backport of patch 1751-secrets-read-permission.sec.patch to authentik-2026.8 (#25965)
- security: automated internal backport of patch 1877-group-hierarchy-roles.sec.patch to authentik-2026.8 (#25966)
- security: automated internal backport of patch 1914-saml.sec.patch to authentik-2026.8 (#25967)
- security: automated internal backport of patch 2032-authenticator-email-recipient-override.sec.patch to authentik-2026.8 (#25968)
- security: automated internal backport of patch 2190-libxml2-doctype.sec.patch to authentik-2026.8 (#25969)
- tenants/settings: fix `flags` response (cherry-pick #25616 to version-2026.8) (#25828)
- web/admin: align command palette navigation with sidebar (cherry-pick #25732 to version-2026.8) (#25733)
- web/admin: fix schedule form (cherry-pick #25778 to version-2026.8) (#25830)
- web/admin: oauth provider: explain what happens when no signing key is selected (cherry-pick #25756 to version-2026.8) (#25769)
- web: fix switches in table toolbars (cherry-pick #25730 to version-2026.8) (#25737)
- web: preserve custom event action labels (cherry-pick #25655 to version-2026.8) (#25833)
- web: show pre-selected value in paginated search-selects (cherry-pick #25609 to version-2026.8) (#25612)
