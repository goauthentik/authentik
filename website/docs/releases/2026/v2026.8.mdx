...
This release does not introduce new configuration options, but it more strictly enforces trusted proxy configuration. Review the [breaking change](../../install-config/reverse-proxy.mdx#forwarded-headers-are-now-restricted-to-trusted-proxies) before upgrading. You can follow the upgrade instructions below; for more detailed information about upgrading authentik, refer to our [Upgrade documentation](../../install-config/upgrade.mdx).
