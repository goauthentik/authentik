...
```shell
wget -O compose.yml https://goauthentik.io/version/2026.8/lifecycle/container/compose.yml
docker compose up -d
```
...
- enterprise/providers/ws\_fed: add SAML 1.1 support (#23851)
...
- stages/authenticator\_duo: clarify duplicate Duo user enrollment (#23588)
- stages/authenticator\_validate: fix intermittently failing throttling test (#24568)
- stages/authenticator\_validate: skip flaky test (#24554)
- stages/authenticator\_webauthn: remove prevent_duplicate_devices (#24450)
- stages/authenticator\_webauthn: Update FIDO MDS3 & Passkey aaguid blobs (#22772)
- stages/authenticator\_webauthn: Update FIDO MDS3 & Passkey aaguid blobs (#23562)
- stages/authenticator\_webauthn: Update FIDO MDS3 & Passkey aaguid blobs (#24039)
- stages/authenticator\_webauthn: Update FIDO MDS3 & Passkey aaguid blobs (#24487)
...
- stages/user\_login: refuse to log in an unsaved pending user (#21691)
- stages/user\_write: only write user when data changes (#24158)
...
- tests/openid\_conformance: Add more conformance coverage (#22174)
...
- web: Fix `content_left`/`content_right` layouts. (#25025)
...
- core: explicitly check `is_active` in auth backends (#25729)
- core: fix bulk session revocation (#25816)
- lib: avoid DNS resolution in `fqdn_rand` (#25728)
- lib/expression: fix PickleError due to resolver match when using `ak_call_policy` (#25763)
- lifecycle: fix server prometheus metrics getting registered early with pid instead of worker ID (#25883)
- packages/django-dramatiq-postgres: prevent crashing consumer thread on schedules with unknown actors (#25656)
- rbac: guard for non-existent object in object permission API (#25601)
...
- tenants/settings: fix `flags` response (#25616)
- web/admin: align command palette navigation with sidebar (#25732)
- web/admin: fix schedule form (#25778)
- web/admin: oauth provider: explain what happens when no signing key is selected (#25756)
- web: fix switches in table toolbars (#25730)
- web: preserve custom event action labels (#25655)
- web: show pre-selected value in paginated search-selects (#25609)
...
- packages/ak-axum/extract/client\_ip: support X-Forwarded-For with ports (#25682)
...
### authentik (v 2026.8.3)

**Notice**: There are breaking changes with authentik v 2026.5
...
* Changed content type : `application/json`
    ...
    * Changed property `invalidation_flow` (string)
...
* Changed content type : `application/json`
    ...
    * Changed property `oidc_auth_providers_obj` (array)
