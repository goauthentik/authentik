...
**SAML unified endpoints**: Instead of an individual endpoint for login and logout for each protocol, there is now a single SAML endpoint for providers that handles login and logout for all protocols.
...
We've removed 17 packages from authentik. Fewer dependencies mean less code to maintain and keep patched, and a smaller attack surface overall.
