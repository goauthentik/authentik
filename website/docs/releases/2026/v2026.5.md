...
Authenticator validation can now throttle repeated failed attempts for email and SMS OTP devices, extending the same brute-force protection already available for TOTP and static authenticators. Admins can tune throttling behavior on the Authenticator Validation stage to slow down repeated guessing attempts without changing the user's login flow.
