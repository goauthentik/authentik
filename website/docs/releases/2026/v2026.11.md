...
Administrators can now require a user to complete one or more flows the next time they log in, before their session is created, by setting the `goauthentik.io/user/next-actions` attribute on the user. Typical uses are forcing a password change or requiring the user to set up an authenticator, and each action is cleared automatically once the user completes it.
