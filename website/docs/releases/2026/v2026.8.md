...
- **Podman support**: authentik now includes a [Podman with Quadlet installation](../../install-config/install/podman-quadlet.mdx) guide. Outposts can also use rootful (`/run/podman/podman.sock`) and rootless (`$XDG_RUNTIME_DIR/podman/podman.sock`) Podman sockets.
