...
- **User security notifications**: New optional email notifications keep users informed about security-relevant changes to their own account: password changes, MFA device changes, blocked logins, account lockdown, scheduled account offboarding, and a welcome email for newly created accounts.
...
authentik no longer creates the `static_authenticator_disable` event. Removing any MFA device, including static authenticators, now creates the generic `mfa_device_deleted` event. Update any Event Matcher policies or notification rules that reference the removed action.
...
authentik now ships notification rules that email users about security-relevant changes to their own account: interactive password changes, MFA devices being added or removed, logins blocked because of impossible travel, [account lockdown](../../security/account-lockdown.md), scheduled [user offboarding](../../sys-mgmt/user-offboarding.md), and a welcome email when their account is created. The rules are disabled by default; to learn how to enable and customize them, refer to the [user security notifications documentation](../../sys-mgmt/events/user-security-notifications.md).

The feature is built on new primitives that are also available to custom notification rules:

- New event types — `user_created`, `login_blocked`, `mfa_device_created`, and `mfa_device_deleted` — with context fields, such as the origin of a change, that notification policies can match on.
