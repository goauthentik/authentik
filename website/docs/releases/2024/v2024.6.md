...
With this release, authentik now enforces unique group names. Existing groups with name collisions can still exists, but new groups will need a unique name. If changing an existing group with a name collision, that group will also need to have its name changed.
