...
-   **Audit log** <span class="badge badge--primary">Enterprise</span> See what fields were changed when objects are updated
...
-   **Tenants have been renamed to brands**
    ...
    Tenants, which used to allow the changing of branding configuration and default flows and a couple other settings depending on which domain authentik was accessed from, have been renamed to _brands_. This is due to this release including full tenancy (see [here](#new-features))
    ...
    See documentation for _brands_ [here](../../core/brands.md)

    Also, **the event retention settings configured in brands (previously tenants, see above) has been removed and is now a system setting**, managed in the admin interface or via the API (see above).
...
-   **Required `offline_access` scope for Refresh tokens**
    ...
    Applications that require will need their configuration update to include the `offline_access` scope mapping.
...
-   **Redis and cache configuration options have been improved**
    ...
    Settings have also been added to configure the Redis instance/database used for tasks and websockets seperately from cache. See [here](../../installation/configuration.mdx#redis-settings).
...
-   **User sessions will be invalidated after this upgrade.**
    ...
    As such, users will need to log back in. This will occur only once and does not require manual action from the user as they will be logged out automatically after the upgrade.
...
-   **Session location and network binding**
    ...
    Sessions for any users can now be bound to a specific geolocation (Continent, Country, City) or network (Autonomous System, subnet, IP address). If the session is accessed from a different location/network it was initially created from, the session will be terminated.
...
-   **Simplified workflow for creating applications and providers**
    ...
    Applications and providers can now be created at the same time using the **Application Wizard**, found on the **Applications** -> **Applications** page of the admin interface, skipping the linking step between the two.
...
-   **Flows can now be restricted to outposts**
    ...
    You can now restrict a flow to be used only by an outpost using the **Require Outpost** setting on the flow. This would mainly be used to LDAP flows.
