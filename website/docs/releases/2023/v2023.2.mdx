...
- Generated avatars, multiple avatar modes
    ...
    authentik now supports multiple avatar modes, and will use the next configured mode when a mode doesn't have an avatar. For example, the new default configuration attempts to use gravatar, but if the user's email does not have a gravatar setup, it will instead use the new generated avatars. See [Configuration](../../sys-mgmt/settings.mdx#avatars)
...
- web/admin: add notice for user_login stage session cookie behavior
...
- web/user: revert truncate behavior for application description

## Fixed in 2023.2.3

- \*: fix [CVE-2023-26481.md](../../security/cves/CVE-2023-26481.mdx), Reported by [@fuomag9](https://github.com/fuomag9)
