...
Roles are a way to simplify the assignment of permissions. Roles are also the backbone of role-based access control (RBAC), an industry standard for managing [access control](../access-control).
...
The easiest workflow for setting up these new users involves [creating a role](./manage-roles.md#create-a-role) specifically for their type of work, and then [assigning that role to a group](./manage-roles.md#assign-a-role-to-a-group) to which all of the users belong.
