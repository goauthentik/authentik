...
-   `AUTHENTIK_POSTGRESQL__PGBOUNCER_CONN_MAX_AGE`: PostgreSQL connection lifetime when using PgBouncer. Defaults to unlimited. If using PgBouncer to handle persistent connections, this should be disabled with `0`
...
All PostgreSQL settings, apart from `USE_PGBOUNCER`, `USE_PGPOOL`, and `PGBOUNCER_CONN_MAX_AGE`, support hot-reloading. Adding and removing read replicas doesn't support hot-reloading.
