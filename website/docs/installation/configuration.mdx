...
-   `AUTHENTIK_STORAGE__MEDIA__S3__SECURE_URLS`: Whether URLS created for users use `https` rather than `http`. Defaults to `true`.
...
Enable the ability for users to change their names, defaults to `true`.
...
Enable the ability for users to change their email addresses, defaults to `false`.
