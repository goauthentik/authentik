...
-   `AUTHENTIK_RESULT_BACKEND__URL`: Result backend configuration URL, defaults to Redis using [the respective settings](#redis-settings)

## Cache Settings

-   `AUTHENTIK_CACHE__BACKEND`: Class to use as cache backend, defaults to `django_redis.cache.RedisCache`
-   `AUTHENTIK_CACHE__URL`: Cache configuration URL, defaults to Redis using [the respective settings](#redis-settings)
...
-   `AUTHENTIK_CHANNEL__BACKEND`: Class to use as channel backend, defaults to `channels_redis.core.RedisChannelLayer`
-   `AUTHENTIK_CHANNEL__URL`: Channel configuration URL, defaults to Redis using [the respective settings](#redis-settings)
