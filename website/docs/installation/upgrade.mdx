...
-   You need to upgrade in sequence; do not skip directly to the most recent version. For example, if you are currently running 2023.10.3, then you will need to upgrade to 2024.2.x then 2024.4.x and then 2024.6.x, in sequence.
...
<Tabs groupId="platform">
  <TabItem value="docker-compose" label="Docker Compose" default>
   ...
   Download the `docker-compose.yml` file using either   `wget -O docker-compose.yml https://goauthentik.io/docker-compose.yml` oe `curl -O https://goauthentik.io/docker-compose.yml`or similar process.
...
If you run the upgrade commands but your version on the Dashboard doesn’t change, folow this steps:

1. Look at the server logs and search for an entry of `migration inconsistency`.
2. If you see this entry, revert to your db backup.
