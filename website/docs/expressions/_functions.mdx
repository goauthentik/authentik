...
### `ak_send_email(address: str, subject: str, body: str = None, stage: EmailStage = None, template: str = None, context: dict = None) -> bool`:ak-version[2025.10]

Send an email using authentik's email system.

The `address` parameter specifies the recipient email address. The `subject` parameter sets the email subject line.
...
The function returns `True` if the email was queued successfully, `False` otherwise.

Examples:

```python
# Send email with plain body
...
# Send email with custom context for template
ak_send_email(
    "user@example.com",
    "Account Update",
