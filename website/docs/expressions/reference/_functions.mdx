...
:::info Error handling
If the called policy raises an exception, `ak_call_policy` does not propagate it. A failing `PolicyResult`
is returned (`.passing` will be `False`), with the error message available in `.messages`.

If no policy matches _name_, a `ValueError` is raised and the **calling** policy fails.
:::
...
### `ak_send_email(address: str | tuple[str, str] | list[str | tuple[str, str]], subject: str, body: str = None, stage: EmailStage = None, template: str = None, context: dict = None, cc: str | tuple[str, str] | list[str | tuple[str, str]] = None, bcc: str | tuple[str, str] | list[str | tuple[str, str]] = None) -> bool`:ak-version[2025.10]
...
- A formatted address string that includes a display name: `"John Doe <user@example.com>"`:ak-version[2026.11]
- A `(name, email)` tuple: `("John Doe", "user@example.com")`:ak-version[2026.11]
- A list of any of the above: `["user1@example.com", ("Jane Doe", "user2@example.com")]`

When a display name is provided, it is included in the `To` or `CC` header of the email, matching the format used by the Email stage. Recipients in `bcc` are never written to a header, so their display name is only used for the delivery address.
...
# Send email with a display name
ak_send_email((request.user.name, request.user.email), "Welcome!", body="Welcome to our platform!")
ak_send_email("John Doe <user@example.com>", "Welcome!", body="Welcome to our platform!")
