...
Authentication is only possible if linux is aware of the authentik user, which can be achieved with [libnss-authentik](#libnss-authentik)
...
2. Navigate to **Directory** > **Token and App passwords** and click **Create** and set the following configuration:
    ...
    - **User**: `akadmin` or another administrator account {temp}
    - **Intent**: `API Token`
    - **Expiring**: disabled
