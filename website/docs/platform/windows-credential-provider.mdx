...
:::warning
Windows Credential Provider is only supported on Windows 11.
:::
...
Now you'll need to install and configure Windows Credential Provider on the Windows device that you want to log in to.

### Install Windows Credential Provider
...
2. Under **Windows Credential Provider packages** click on **Windows** to download the Windows Credential Provider MSI file.
3. Once the download is complete, install the MSI file.
4. Confirm that authentik Windows Credential Provider is installed by opening a Powershell or Terminal window and entering the following command: (todo)
