...
Allows you to SSH to Linux hosts using authentik credentials or optionally log in to the Windows device if Windows Credential Provider is installed.
...
- The password of the Windows user account that's logged in to is set to a random string when WCP is enabled.
