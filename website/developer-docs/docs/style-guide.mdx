...
-   Try to order the documentation sections in an order that makes it easiest for the user to configure. That is, order the sections in the same way the actual workflow to accomplish the task is ordered. Use headings (sub-titles) to break up long documents, and make it easier to find a specific section.
...
-   For variables or placeholders use _italic_ font for the variable, place angle brackets around it, and use place-holder names that makes it obvious that the user needs to replace it.

    Example: <kbd>https://<em>&lt;company-domain&gt;</em>/source/oauth/callback/<em>&lt;source-slug&gt;</em></kbd>

    Make sure to specify if the placeholder is something the user needs to define, is system-defined or generated.
...
-   Both titles and headers (H1, H2, H3) use sentence style capitalization, meaning that only the first word is capitalized. However, if the title or header includes a proper noun (name of a product, etc) then capitalize those words.
    Examples: - Configure your provider - Configure the Google Workspace provider
