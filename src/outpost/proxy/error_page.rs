...
/// Build an HTML error response with the given status, title, and message.
pub(super) fn error_response(status: StatusCode, title: &str, message: &str) -> Response {
    match (ErrorPage { title, message }).render() {
...
mod tests {
    ...
    fn renders_and_escapes() {
        ...
        assert!(page.contains("<h1>Bad Gateway</h1>"));
        // askama escapes with numeric character references.
        assert!(page.contains("boom &#60;script&#62;alert(1)&#60;/script&#62;"));
