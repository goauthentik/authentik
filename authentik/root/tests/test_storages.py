...
class TestS3Storage(TestCase):
    ...
    def test_url_with_secure_urls_true(self, mock_bucket, mock_secure_urls, mock_normalize_name):
        ...
        mock_normalize_name.return_value = "media/public/icons/logo.svg"
        mock_secure_urls.return_value = True
        ...
        # Check that the final URL uses HTTPS
        self.assertTrue(url.startswith("https://"))

        # Check the URL contains the expected domain and path
        expected_url_parts = [
            "https://example-bucket.s3.region.com",
            "media/public/icons/logo.svg",
            "X-Amz-Algorithm=AWS4-HMAC-SHA256",
        ]
    ...
    def test_url_with_secure_urls_false(self, mock_bucket, mock_secure_urls, mock_normalize_name):
        ...
        # Check that the final URL uses HTTP
        self.assertTrue(url.startswith("http://"))

        # Check the URL contains the expected domain and path
        expected_url_parts = [
            "http://example-bucket.s3.region.com",
            "media/public/icons/logo.svg",
            "X-Amz-Algorithm=AWS4-HMAC-SHA256",
        ]
        for part in expected_url_parts:
