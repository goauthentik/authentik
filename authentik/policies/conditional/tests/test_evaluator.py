...
class TestConditionalEvaluator(TestCase):
    ...
    def test_pickle(self):
        """Compiled policies are pickled with policy results, for example to cache them or to
        send them from the policy process, and must not fail because of resolvers"""
        policy = ConditionalPolicy(
            name=generate_id(),
            actions=tree(
                group(
                    "all",
                    cond("user.email", "is_set"),
                    cond("plan.is_sso", "is_false"),
        ...
        self.request.context["flow_plan"] = FlowPlan(generate_id())
        result = policy.passes(self.request)
        self.assertTrue(result.passing)
        restored = loads(dumps(policy))  # nosec
        self.assertEqual(restored.compiled(), policy.compiled())
        for variable in registry.variables.values():
            self.assertIs(loads(dumps(variable)), variable)  # nosec
        for setter in registry.setters.values():
            self.assertIs(loads(dumps(setter)), setter)  # nosec
        for operator in OPERATORS.values():
            self.assertIs(loads(dumps(operator)), operator)  # nosec
    ...
    def test_logic(self):
        ...
        self.assertTrue(self.passes(group("none", false)))
    ...
    def test_missing(self):
        ...
        guarded = group(
            "any", cond("plan.context", "is_not_set", param="username", cast="string"), node
        )
