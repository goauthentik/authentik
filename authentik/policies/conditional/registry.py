...
class Variable:
    ...
    def __reduce__(self):
        # Resolvers can be lambdas, which can't be pickled. Variables are registered on
        # startup in every process, so they're pickled by key, for example as part of a
        # compiled policy in a cached policy result.
        return (_registered_variable, (self.key,))
...
class Setter:
    ...
    def __reduce__(self):
        # See `Variable.__reduce__`
        return (_registered_setter, (self.key,))
...
class ConditionalPolicyRegistry:
    ...
    def add_variable(  # noqa: PLR0913
        ...
        *,
        ...
        """Register a variable with an existing resolver, for example from `attribute()` or
        `context_key()`"""
        ...
        self.variables[key] = Variable(
            ...
            module=module or sys._getframe(1).f_globals.get("__name__", ""),
        )
...
def _registered_variable(key: str) -> Variable:
    return registry.variables[key]


def _registered_setter(key: str) -> Setter:
    return registry.setters[key]


def attribute(
    source: Callable[[PolicyRequest], Any], name: str, blank_missing: bool = False
) -> Callable[[PolicyRequest], Any]:
    """Resolver for the attribute `name` of the object `source` returns for a request. The
    value is `MISSING` when there is no object, or the attribute is None (or empty, with
    `blank_missing`)."""
    ...
    def resolve(request: PolicyRequest) -> Any:
        ...
        if value is None or (blank_missing and not value):
            return MISSING
        return value
...
def context_key(
    ...
    """Resolver for a key of the policy request's context, which in flows contains the
    flow context. With `instance_of`, other values are treated as `MISSING`."""
