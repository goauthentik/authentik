...
# This is just a nicer way to tell people about the new group name uniqueness requirement than them
# facing a cold, hard `django.db.utils.IntegrityError`.
def error_on_duplicate_group_name(apps: Apps, schema_editor: BaseDatabaseSchemaEditor):
    db_alias = schema_editor.connection.alias
    AuthentikGroup = apps.get_model("authentik_core", "Group")

    duplicates = (
        AuthentikGroup.objects.using(db_alias)
        .values("name")
        .annotate(Count("name"))
    ...
    if len(duplicates):
        for group in duplicates:
            LOGGER.error(
                "Group with duplicate name detected", name=group["name"], count=group["name__count"]
            )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(error_on_duplicate_group_name),
        migrations.AddField(
            model_name="user",
            name="roles",
