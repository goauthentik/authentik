...
@actor(description=_("Purge Google Workspace provider objects."))
def google_workspace_purge_objects(*args, **kwargs):
...
def google_workspace_sync(provider_pk: int, *args, **kwargs):
    ...
    return sync_tasks.sync(
        provider_pk, google_workspace_sync_objects, google_workspace_purge_objects
    )
