...
from authentik.policies.models import RequestableMixin


def user_can_request(pbm: RequestableMixin, user: User, request: Request) -> bool:
    """Whether `user` is eligible to request access to `pbm`, per the
    PolicyBindingModelRequestRule(s) attached to it. An object with no rule attached at all
...
def _requestable(view: GenericViewSet, request: Request) -> list[RequestableMixin]:
