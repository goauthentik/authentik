...
class TestProxySessionManager(TestCase):
    ...
    def test_cleanup_expired_boundary_conditions(self):
        ...
        self.assertTrue(
            ProxySession.objects.filter(session_key="expires-one-second-future").exists()
        )
    ...
    def test_cleanup_expired_with_custom_manager(self):
        ...
        self.assertTrue(callable(ProxySession.objects.cleanup_expired))
    ...
    def test_proxy_session_manager_update_methods(self):
        ...
        # Test update method
        updated_count = ProxySession.objects.filter(session_key="update-test").update(
            data=b"updated-data"
        )
    ...
    def test_proxy_session_manager_delete_methods(self):
        ...
        self.assertEqual(deleted_count, 1)
        self.assertFalse(ProxySession.objects.filter(pk=session1.pk).exists())
        self.assertTrue(ProxySession.objects.filter(pk=session2.pk).exists())
        ...
        self.assertFalse(ProxySession.objects.filter(pk=session2.pk).exists())
