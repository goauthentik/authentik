...
class TestRACConnections(APITestCase):
    ...
    def test_reconnect_same_endpoint(self):
        ...
        old_token_exists = ConnectionToken.filter_not_expired().filter(pk=token1_id).exists()
        self.assertFalse(old_token_exists)

        # Verify we have exactly one new token
        tokens = ConnectionToken.filter_not_expired().filter(endpoint=self.endpoint1)
        self.assertEqual(tokens.count(), 1)
    ...
    def test_websocket_groups(self, mock_get_channel_layer):
        ...
        # Connect
        with patch("authentik.providers.rac.consumer_client.RACClientConsumer.accept"):
            consumer.connect()

        # Verify the consumer joined the correct groups
        mock_layer.group_add.assert_any_call(
            f"group_rac_session_{self.session.session_key}_endpoint_{self.endpoint1.pk}",
            "test_channel",
        )
        ...
        mock_layer.group_discard.assert_any_call(
            f"group_rac_session_{self.session.session_key}_endpoint_{self.endpoint1.pk}",
            "test_channel",
        )
        self.assertFalse(ConnectionToken.objects.filter(pk=token.pk).exists())

    def test_maximum_connections(self):
        """Test maximum connections limit for an endpoint"""
        # Set maximum connections to 1 for endpoint1
