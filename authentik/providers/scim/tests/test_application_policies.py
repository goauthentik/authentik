...
class SCIMApplicationPoliciesTests(TestCase):
    ...
    def test_static_bindings_dont_invoke_policy_engine_per_user(self):
        """Performance regression test (PR #13947 / issue: 15-20 min per task).
        ...
        production instance with ~6000 users multiplied across ~60 paginated
        sync tasks produced ~360,000 PolicyEngine evaluations per provider per
        full sync.
        """
        PolicyBinding.objects.create(target=self.app, group=self.group1, order=0)
        ...
        for _ in range(20):
            ...
            user.ak_groups.add(self.group1)

        with patch("authentik.providers.scim.models.PolicyEngine") as mock_policy_engine:
        ...
        self.assertEqual(
            ...
            0,
            (
                "PolicyEngine was instantiated "
                f"{mock_policy_engine.call_count} times during get_object_qs(). "
                "Static bindings (no Policy object) must be evaluated as a "
                "single SQL filter, not per-user. This is the regression that "
                "caused 15-20 min sync tasks for instances with thousands of "
                "users."
            ),
        )
    ...
    def test_static_bindings_query_count_independent_of_user_count(self):
        ...
        count grows with the user count -- the previous (broken) implementation
        ran a PolicyEngine evaluation per user, multiplying the query count by
        the user count.
        """
        PolicyBinding.objects.create(target=self.app, group=self.group1, order=0)
    ...
    def test_actual_policy_binding_falls_back_to_per_user_evaluation(self):
        ...
        # Create a policy that always passes
