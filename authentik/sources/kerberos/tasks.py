...
from authentik.sources.kerberos.sync import KerberosSynchronizer

LOGGER = get_logger()
SYNC_CLASSES = [
    KerberosSynchronizer,
]
CACHE_KEY_PREFIX = "goauthentik.io/sources/kerberos/"
...
def kerberos_sync_all():
    """Sync all sources"""
    for source in KerberosSource.objects.filter(enabled=True):
...
def kerberos_sync_single(source_pk: str):
    ...
    with source.sync_lock as lock_acquired:
        ...
        DBSystemTask.objects.filter(name="kerberos", uid__startswith=source.slug).delete()
        task = chain(
