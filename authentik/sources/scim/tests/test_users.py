...
class TestSCIMUsers(APITestCase):
    ...
    def test_user_update_patch_nested(self):
        """Test user update (patch) of an attribute nested in an existing object"""
        user = create_test_user()
        existing = SCIMSourceUser.objects.create(
            source=self.source,
            user=user,
            ...
            attributes={
                "userName": generate_id(),
                SCIM_URN_USER_ENTERPRISE: {"department": "a"},
            },
        )
        response = self.client.patch(
            reverse(
            ...
            data=dumps(
                {
                    ...
                    "Operations": [
                        {
                            "op": "replace",
                            "path": f"{SCIM_URN_USER_ENTERPRISE}:department",
                            "value": "b",
                        }
                    ],
                }
        ...
        self.assertEqual(response.status_code, 200)
        existing.refresh_from_db()
        self.assertEqual(existing.attributes[SCIM_URN_USER_ENTERPRISE], {"department": "b"})
