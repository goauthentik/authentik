 ...
 *
 *   - `script-src 'unsafe-inline'`, because the generated document carried inline `<script>` blocks
 *     for the provider callbacks and the `postMessage` bridge.
 *   - `frame-src blob:`, because that document was served to the iframe as a blob URL.
 *   - `document.write()` into `about:blank` for reCAPTCHA and hCaptcha, whose domain verification
 *     could not see the real origin through a blob URL. None of those remain. Providers are
 *     configured by passing callbacks to their own render APIs, so no markup is generated and
 *     nothing is evaluated from a string. What a policy still has to allow is per-provider and
 *     derived from the stage's configured `js_url` / `api_url`, which is why it belongs on the
 *     server rather than hardcoded here:
 *   - `script-src` — the origin of `js_url`.
 *   - `frame-src` — the provider's challenge origin. Usually the `js_url` origin; reCAPTCHA also
 *     frames `https://www.google.com`.
 *   - `connect-src` — the origin the widget reports back to, generally the `js_url` origin.
 *   - `style-src 'unsafe-inline'` — reCAPTCHA and hCaptcha both set inline styles on the elements
 *     they inject. Turnstile and Cap do not. Note that `api_url` is called by the authentik server,
 *     not the browser, so it never needs a CSP entry.
