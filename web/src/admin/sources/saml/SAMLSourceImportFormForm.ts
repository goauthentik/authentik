...
import { FlowDesignationEnum, type SourcesSamlImportMetadataCreateRequest } from "@goauthentik/api";
...
export function renderForm(source: Partial<SourcesSamlImportMetadataCreateRequest> = {}) {
    return html`
        ...
        <ak-form-element-horizontal label=${msg("Authentication flow")} name="authenticationFlow">
        ...
        <ak-form-element-horizontal label=${msg("Enrollment flow")} name="enrollmentFlow">
            ...
            ></ak-source-flow-search>
            <p class="pf-c-form__helper-text">${msg("Flow to use when enrolling new users.")}</p>
        </ak-form-element-horizontal>
    `;
}
