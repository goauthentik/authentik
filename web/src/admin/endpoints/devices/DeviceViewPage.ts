                ...
                <div class="pf-l-grid__item pf-m-4-col pf-c-card">
                    ...
                    <div class="pf-c-card__body">
                        ${renderDescriptionList([
                            [
                                msg("Name"),
                                this.device.facts.network?.hostname,
                            ],
                            [
                                msg("Serial number"),
                                this.device.facts.hardware?.serial,
                            ],
                            [
                                msg("Operating system"),
                                [this.device.facts.os?.name, this.device.facts.os?.version].join(" "),
                            ],
                            [
                                msg("Disk encryption"),
                                html`<ak-status-label ?good=${rootDisk.encryptionEnabled}></ak-status-label>`,
                            ],
                            [
                                msg("Firewall enabled"),
                                html`<ak-status-label ?good=${this.device.facts.network?.firewallEnabled}></ak-status-label>`,
                            ],
                        ], {horizontal: true})}
                    </div>
                </div>
                <div class="pf-l-grid__item pf-m-4-col pf-c-card">
                    ...
                    <div class="pf-c-card__body">
                        ${renderDescriptionList([
                            [
                                msg("Manufacturer"),
                                this.device.facts.hardware?.manufacturer,
                            ],
                            [
                                msg("Model"),
                                this.device.facts.hardware?.model,
                            ],
                            [
                                msg("CPU"),
                                msg(str`${this.device.facts.hardware?.cpuCount} x ${this.device.facts.hardware?.cpuName}`),
                            ],
                            [
                                msg("Memory"),
                                this.device.facts.hardware?.memoryBytes ? getSize(this.device.facts.hardware?.memoryBytes) : "-",
                            ],
                            [
                                msg("Disk usage"),
                                html`<progress
                        ...
                        <dl class="pf-c-description-list pf-m-horizontal">
                            ${this.device.connectionsObj.map(conn => {
                                ...
                                <dt class="pf-c-description-list__term">
                                    <span class="pf-c-description-list__text">${conn.connectorObj.name}</span>
                                </dt>
                                <dd class="pf-c-description-list__description">
                                    <div class="pf-c-description-list__text">
                                        ${conn.latestSnapshot?.created ? Timestamp(conn.latestSnapshot.created) : html`-`}
                                    </div>
                                </dd>
                            </div>`;
                            })}
