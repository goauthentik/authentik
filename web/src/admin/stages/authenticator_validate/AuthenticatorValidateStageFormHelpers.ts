...
export function stagesSelector(instanceStages: string[] | undefined) {
    if (!instanceStages) {
        return async (stages: DualSelectPair<Stage>) =>
            stages.filter(([_0, _1, _2, stage]: DualSelectPair<Prompt>) => stage !== undefined);
    }
    return async () => {
        const stagesApi = new StagesApi(DEFAULT_CONFIG);
        const stages = await Promise.allSettled(
            instanceStages.map((instanceId) => stagesApi.stagesAllRetrieve({ stageUuid: instanceId }))
        );
        return stages
...
export async function authenticatorWebauthnDeviceTypesListProvider(page = 1, search = "") {
    const devicetypes = await new StagesApi(DEFAULT_CONFIG).stagesAuthenticatorWebauthnDeviceTypesList({
        pageSize: 20,
        search: search.trim(),
        page,
    });
