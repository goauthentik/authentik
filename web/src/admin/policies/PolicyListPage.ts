            ...
            html`<div>
                ...
                ${(item.boundTo || 0) > 0
                    ? html`<i class="pf-icon pf-icon-ok"></i>
                          <small>${msg(str`Assigned to ${item.boundTo} object(s).`)}</small>`
                    : html`<i class="pf-icon pf-icon-warning-triangle"></i>
                          <small>${msg("Warning: Policy is not assigned.")}</small>`}
            </div>`,
    ...
    renderToolbar(): TemplateResult {
        return html` ${super.renderToolbar()}
            ...
            >
                ...
                <p slot="body">
                    ${msg(`Are you sure you want to clear the policy cache?
