...
export class AkPolicyActionBuilder extends AKControlElement<PolicyActions> {
    static styles = [
        ...
        css`
            ...
            ak-toggle-group::part(toggle-group) {
                    ...
                    ? html`<ak-chip-group>
                          ${values.map(
                              (value, idx) =>
                                  html`<ak-chip
                                      removable
                                      @remove=${() => onChange(values.filter((_, i) => i !== idx))}
                                      >${label(value)}</ak-chip
                                  >`,
                          )}
                      </ak-chip-group>`
    ...
    protected renderOperandEditor(
        ...
        const source = html`<ak-toggle-group
            value=${isVariable ? "variable" : "literal"}
            @ak-toggle=${(ev: ToggleGroupEvent) =>
                onChange(
                    ev.value === "variable"
    ...
    protected renderCompareEditor(node: CompareNode) {
        ...
        return html`<div class="pf-c-form__group">
                <span class="pf-c-form__label"
                    >${msg("Value", { id: "policies.actions.compare.value" })}</span
                >
                ...
                ${variable ? html`<span class="pf-c-form__helper-text">${variable.description}</span>` : nothing}
                ${
            ...
            <div class="pf-c-form__group">
                <label class="pf-c-form__label" for="operator">
    ...
    protected renderGroupEditor(node: GroupNode) {
        return html`<div class="pf-c-form__group">
            <span class="pf-c-form__label"
                >${msg("Passes when", { id: "policies.actions.group.label" })}</span
            >
            <ak-toggle-group
                value=${node.op}
                @ak-toggle=${(ev: ToggleGroupEvent<GroupNode["op"]>) => {
                    node.op = ev.value;
                    this.changed(node);
                }}
            >
                ${(["all", "any", "none"] as const).map(
                    (op) => html`<option value=${op}>${GROUP_LABELS[op]}</option>`,
                )}
            </ak-toggle-group>
    ...
    protected renderPolicyEditor(node: PolicyNode) {
        return html`<div class="pf-c-form__group">
            <span class="pf-c-form__label"
                >${msg("Policy", { id: "policies.actions.policy.label" })}</span
            >
            <ak-search-select
    ...
    protected renderSetEditor(action: SetAction) {
        ...
        return html`<div class="pf-c-form__group">
                <label class="pf-c-form__label" for="setter"
                    >${msg("Set", { id: "policies.actions.set.target" })}</label
                >
                <select
                    id="setter"
                    class="pf-c-form-control"
                ...
                ${setter ? html`<span class="pf-c-form__helper-text">${setter.description}</span>` : nothing}
            </div>
            ${
                setter && setter.param !== ConditionParamKindEnum.None
                    ? html`<div class="pf-c-form__group">
                          <label class="pf-c-form__label" for="setter-key">
    ...
    protected renderStopEditor(action: StopAction) {
        return html`<div class="pf-c-form__group">
                <span class="pf-c-form__label"
                    >${msg("Result", { id: "policies.actions.stop.result" })}</span
                >
                <ak-toggle-group
                    value=${action.result}
                    @ak-toggle=${(ev: ToggleGroupEvent<StopAction["result"]>) => {
                        action.result = ev.value;
                        this.changed(action);
                    }}
                >
                    <option value="pass">
            ...
            <div class="pf-c-form__group">
                <label class="pf-c-form__label" for="stop-message">
        ...
        const item = <T>(type: BlockType<T>, iconClass = "") =>
            ...
            >
                ...
                <span>
                    ...
                    <span class="pf-c-form__helper-text">${type.description}</span>
                </span>
            </button>`;
    ...
    protected renderMenu(entries: [string, string, () => void, boolean?][]) {
        return html`<ak-dropdown class="pf-c-dropdown">
            ...
            <menu class="pf-c-dropdown__menu pf-m-align-right" hidden>
                ${entries.map(([icon, label, action, hidden]) =>
                    icon === "-"
                        ? html`<li class="pf-c-divider" role="separator"></li>`
                        : html`<li ?hidden=${hidden}>
                              <button
                                  type="button"
                                  class="pf-c-dropdown__menu-item pf-m-icon ${
                                      icon === "fa-trash" ? "danger" : ""
                                  }"
                                  @click=${() => action()}
                              >
    ...
    protected renderCard(
        ...
        >
            ...
            >
                ...
                ${
                    disabled
                        ? html`<span class="pf-c-label pf-m-compact">
                              <span class="pf-c-label__content">
                                  ${msg("Disabled", { id: "policies.actions.card.disabled" })}
                              </span>
                          </span>`
                        : nothing
                }
