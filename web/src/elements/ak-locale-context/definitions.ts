...
const debug: LocaleRow = [
    "pseudo-LOCALE",  /^pseudo/i,  () => msg("Pseudolocale (for testing)"),  async () => await import("#locales/pseudo-LOCALE"),
];

// prettier-ignore
const LOCALE_TABLE: LocaleRow[] = [
    ["de",      /^de([_-]|$)/i,      () => msg("German"),                async () => await import("#locales/de")],
    ["en",      /^en([_-]|$)/i,      () => msg("English"),               async () => await import("#locales/en")],
    ["es",      /^es([_-]|$)/i,      () => msg("Spanish"),               async () => await import("#locales/es")],
    ["fr",      /^fr([_-]|$)/i,      () => msg("French"),                async () => await import("#locales/fr")],
    ["it",      /^it([_-]|$)/i,      () => msg("Italian"),               async () => await import("#locales/it")],
    ["ko",      /^ko([_-]|$)/i,      () => msg("Korean"),                async () => await import("#locales/ko")],
    ["nl",      /^nl([_-]|$)/i,      () => msg("Dutch"),                 async () => await import("#locales/nl")],
    ["pl",      /^pl([_-]|$)/i,      () => msg("Polish"),                async () => await import("#locales/pl")],
    ["ru",      /^ru([_-]|$)/i,      () => msg("Russian"),               async () => await import("#locales/ru")],
    ["tr",      /^tr([_-]|$)/i,      () => msg("Turkish"),               async () => await import("#locales/tr")],
    ["zh_TW",   /^zh[_-]TW$/i,       () => msg("Taiwanese Mandarin"),    async () => await import("#locales/zh_TW")],
    ["zh-Hans", /^zh(\b|_)/i,        () => msg("Chinese (simplified)"),  async () => await import("#locales/zh-Hans")],
    ["zh-Hant", /^zh[_-](HK|Hant)/i, () => msg("Chinese (traditional)"), async () => await import("#locales/zh-Hant")],
    debug
];
