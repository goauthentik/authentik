...
describe("sameRouteMatch", () => {
    ...
    it("is true for fresh matches of the same route and params (a search-only change)", () => {
        expect(sameRouteMatch(matchRoute("/users/42", [users]), matchRoute("/users/42", [users]))).toBe(
            true,
        );
    });

    it("is false when the path parameters differ", () => {
        expect(sameRouteMatch(matchRoute("/users/1", [users]), matchRoute("/users/2", [users]))).toBe(
            false,
        );
    });
    ...
    it("is false when the matched route differs", () => {
        expect(
            sameRouteMatch(matchRoute("/users/1", [users, groups]), matchRoute("/groups/1", [users, groups])),
        ).toBe(false);
    });
});
