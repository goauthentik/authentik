/**
 * @file Route definition for path-based routing. A `Route` wraps a `URLPattern` constructed
 *   **once** at construction (never per match) and renders a template for the matched path
 *   parameters. The `name` is a stable identifier used for Sentry span naming and href generation.
 */
 ...
 *
 * Route tables are heterogeneous: one route declares `{ uuid: string }` path parameters, its
 * neighbor declares none. `Route`'s parameter type sits in a contravariant position (the render
 * callback), so `Route<{ uuid: string }>` is _not_ assignable to `Route<RouteParameterRecord>` and
 * a mixed table cannot be typed as `Route[]`.
 *
 * Declaring a table as `RouteLike[]` erases the parameter type through method bivariance: each
 * route keeps its own typed render callback, while the outlet and matcher drive them all uniformly
 * with the untyped groups a `URLPattern` match actually yields.
 */
...
export class Route<P extends RouteParameterRecord = RouteParameterRecord> implements RouteLike {
     ...
     *
     * Unlike {@linkcode render}, this exposes the render callback's result — synchronous or
     * asynchronous — as a promise and lets rejections propagate, so the outlet can own the loading
     * and error states. A synchronous throw in the callback becomes a rejected promise.
     */
