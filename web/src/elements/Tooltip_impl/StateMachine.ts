 ...
 * those states.
 *
 * Each state is responsible for one of two things:
 *
 * 1. *scheduling* a timeout while waiting for any event that will cancel that timeout
 * 2. *transitioning* the dialog from hide to show.
 *
...
export class TooltipInitialState extends TooltipEvents {
    public override readonly type = "tooltip-hidden";
...
/**
 * The tooltip is waiting to to be shown.
 *
 * This delay allows the user to cross the screen without tooltips popping up
 * and hiding like some annoying cartoon. Either the tooltip will be shown,
 * or the show will be cancelled.
 */
class ScheduledShow extends TooltipEvents {
    public override readonly type = "scheduled-show";
...
/**
 * The tooltip is now shown. The pointer or focus has activated one of the elements.
 * If either issues a leave event, we schedule a hide.
 */
class TooltipShown extends TooltipEvents {
    public override readonly type = "tooltip-shown";
...
/**
 * One of the elements scheduled a hide.
 *
 * The tooltip will be told to hide the tooltip after the delay, unless one of
 *  the visible elements cancels the hide due to an activation event.
 */
class ScheduledHide extends TooltipEvents {
    public override readonly type = "hide-scheduled";
