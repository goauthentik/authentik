...
/**
 * The header account switcher (`ak-user-switcher`), which owns both the multi-account entries and
 * the sign-out affordance in the User and Admin interfaces.
 */
export class UserSwitcherFixture extends PageFixture {
    ...
    /**
     * Signs the active account out. Rendered unconditionally, which makes it the sentinel for "the
     * menu is open".
     */
     ...
     *
     * Entries are labelled with the account's display name and email, so an email pattern
     * identifies one unambiguously under any `UserDisplay` setting.
     */
     ...
     * `ak-user-switcher` that rebuilds the dropdown subtree closes an already-open menu. On a
     * freshly loaded page the session response lands right about then, so a single toggle click is
     * not reliably enough to leave the menu open.
     */
     ...
     *
     * Entries cannot be asserted on before the menu is open: the closed `<menu>` carries `hidden`,
     * which drops its whole subtree from the accessibility tree, so `menuitem` queries match
     * nothing at all. Retrying around both the open and the click also absorbs entries that arrive
     * with a late session response.
     */
