 ...
 *
 * Much of the criteria that determines whether a **build** is considered a prerelease is derived
 * from environment variables -- not the SemVer version.
 *
 * Local development builds are also consider prerelease since they're not necessarily tied to a
 * specific SemVer version.
 *
 * If you're developing locally and using a Dockerized development environment, Set the
 * `GIT_BUILD_HASH` environment variable to `dev` to hint that the container should prefer
 * pre-release versioning over its own.
 */
const prerelease = NodeEnvironment === "development" || !GIT_BUILD_HASH || GIT_BUILD_HASH === "dev";

/**
 * An identifier representing the current version of authentik, optionally prefixed with Git commit
 * hash.
 *
