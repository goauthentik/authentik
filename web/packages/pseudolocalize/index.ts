...
const defaultConfig = "./lit-localize.json";

type ExtractProps = { configFile: string };

function extract({ configFile = defaultConfig }: ExtractProps) {
    const baseConfig = JSON.parse(readFileSync(configFile, "utf-8"));
...
program
    .command("extract")
    .description("Extract a pseudolocalization file from your source locale")
    .option("-c, --config <lit-localize config>", "Path to the lit-localize configuration file")
    .action(extract);
