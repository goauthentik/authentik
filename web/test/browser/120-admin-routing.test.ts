...
test.describe("Admin interface routing", () => {
    ...
    test("Deep-load a param route (flow detail)", async ({ session, navigator, page }) => {
        ...
        const firstLink = page
            .locator("ak-flow-list a[href*='/if/admin/flow/flows/']")
            .first();
    ...
    test("Back and forward traverse pushState navigations", async ({ session, navigator, page }) => {
        ...
        await expect(page.locator("ak-admin-overview"), "Overview renders after back").toBeVisible();
    ...
    test("Legacy admin hash URL redirects through the shim", async ({ session, navigator, page }) => {
        await session.login({ to: OVERVIEW });
