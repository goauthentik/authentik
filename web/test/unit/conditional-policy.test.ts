...
describe("validation errors", () => {
    ...
    const tree: ConditionNode = {
        ...
        children: [
            ...
            { type: "group", op: "none", children: [negated] },
        ],
    };
    ...
    it("computes node paths like the API", () => {
        ...
        expect([...paths.values()]).toEqual([
            ...
            "actions.0.condition.children.1.children.0",
        ]);

        expect(paths.get(negated)).toBe("actions.0.condition.children.1.children.0");
    });

    it("assigns errors to the deepest matching node", () => {
        const paths = [...collectNodePaths(tree, "root").values()];

        expect(errorOwner("root.children.1.children.0.operator", paths)).toBe(
    ...
    it("groups messages and prefixes fields", () => {
        ...
        const errors = assignErrors(
            {
                ...
                "root.children.1.children.0.value.variable.key": ["Field required"],
        ...
        expect(errors.get("root.children.1.children.0")).toEqual([
            "value.variable.key: Field required",
        ]);
