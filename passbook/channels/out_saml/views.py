...
from passbook.channels.out_saml.exceptions import CannotHandleAssertion
from passbook.channels.out_saml.models import SAMLOutlet
from passbook.channels.out_saml.processors.types import SAMLResponseParams
...
class AccessRequiredView(AccessMixin, View):
    ...
    _provider: Optional[SAMLOutlet] = None

    @property
    def provider(self) -> SAMLOutlet:
        """Get provider instance"""
        if not self._provider:
            application = get_object_or_404(
                Application, slug=self.kwargs["application"]
            )
            provider: SAMLOutlet = get_object_or_404(
                SAMLOutlet, pk=application.provider_id
            )
...
class LoginBeginView(AccessRequiredView):
    """Receives a SAML 2.0 AuthnRequest from a Service Outlet and
    ...
    def handler(self, source, application: str) -> HttpResponse:
        ...
        try:
            ...
            did_you_mean_link = self.request.build_absolute_uri(
                reverse(
                    "passbook_channels_out_saml:saml-login-initiate",
        ...
        return redirect(
            reverse(
                "passbook_channels_out_saml:saml-login-authorize",
                kwargs={"application": application},
            )
        )
...
class AuthorizeView(AccessRequiredView):
    ...
    Presents a SAML 2.0 Assertion for POSTing back to the Service Outlet."""
...
class SLOLogout(AccessRequiredView):
    """Receives a SAML 2.0 LogoutRequest from a Service Outlet,
...
class DescriptorDownloadView(AccessRequiredView):
    ...
    def get(self, request: HttpRequest, application: str) -> HttpResponse:
        ...
        except Outlet.application.RelatedObjectDoesNotExist:  # pylint: disable=no-member
            return bad_request_message(
                request, "Outlet is not assigned to an application."
            )
        else:
