...
export function instanceOfCurrentBrand(value: object): value is CurrentBrand {
    ...
    if (!('brandingLogoThemedUrls' in value) || value['brandingLogoThemedUrls'] === undefined) return false;
    if (!('brandingFavicon' in value) || value['brandingFavicon'] === undefined) return false;
    if (!('brandingFaviconThemedUrls' in value) || value['brandingFaviconThemedUrls'] === undefined) return false;
...
export function CurrentBrandToJSONTyped(value?: Omit<CurrentBrand, 'branding_logo_themed_urls'|'branding_favicon_themed_urls'|'ui_footer_links'|'ui_theme'|'default_locale'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
