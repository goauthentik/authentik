...
export function instanceOfSAMLSource(value: object): value is SAMLSource {
    ...
    if ((!('ssoUrl' in (value as Record<string, any>)) && !('sso_url' in (value as Record<string, any>))) || ((value as Record<string, any>)['ssoUrl'] === undefined && (value as Record<string, any>)['sso_url'] === undefined)) return false;
    return true;
}
...
export function SAMLSourceToJSONTyped(value?: Omit<SAMLSource, 'pk'|'component'|'verboseName'|'verboseNamePlural'|'metaModelName'|'managed'|'iconUrl'|'iconThemedUrls'|'urlIssuer'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
