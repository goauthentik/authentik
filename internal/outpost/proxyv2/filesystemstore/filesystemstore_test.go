...
func createTempSessionFile(t *testing.T, dir string, modTime time.Time) string {
	...
	err := os.WriteFile(path, []byte("session data"), 0644)
...
func TestSessionCleanup_AlreadyRunning(t *testing.T) {
	...
	err = store.SessionCleanup(ctx)
	assert.ErrorIs(t, err, ErrSessionCleanupAlreadyRunning, "expected ErrSessionCleanupAlreadyRunning")

	// Now unlock and clean up
...
func TestPersistentStore_ReusesStore(t *testing.T) {
	tmpDir := t.TempDir()
	store1 := GetPersistentStore(tmpDir)
	assert.NotNil(t, store1)

	store2 := GetPersistentStore(tmpDir)
