...
func (g *GoUnicorn) initCmd() {
	...
	if !config.Get().Debug {
		...
		if coveragePath, ok := os.LookupEnv("AUTHENTIK_COVERAGE"); ok {
			command = "coverage"
			args = []string{"run", "--data-file", coveragePath, "-m", "gunicorn", "-c", "./lifecycle/gunicorn.conf.py", "authentik.root.asgi:application"}
		} else {
			command = "gunicorn"
